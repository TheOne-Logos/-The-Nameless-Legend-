...
namespace Balance {
    ...
    constexpr double GARRISON_DISTANCE_DECAY_RATE = 0.02;  // 每格距离衰减系数
    constexpr double GARRISON_MIN_RATIO = 0.1;             // 最低驻军比例（相对于人口）
    constexpr double GARRISON_MAX_RATIO = 0.3;             // 最高驻军比例（相对于人口）
    constexpr double GARRISON_CAPITAL_BONUS = 1.5;         // 首都驻军加成
    
    constexpr double DISTANCE_PENALTY_FACTOR       = 50.0;   // 距离每50格，攻击力减半
    constexpr double NON_SETTLEMENT_DEFENSE_RATIO  = 0.1;    // 非定居点防守力量为全国军队的10%
    constexpr double ATTACK_DEPLOYMENT_RATIO       = 0.2;    // 攻击方可调动的军队比例
    constexpr double GARRISON_DEFENSE_BONUS        = 0.5;    // 定居点驻军额外防御加成（每格驻军加成）
    
    // ===== 灾害恢复常量 =====
    constexpr int DISASTER_RECOVERY_TURNS = 20;      // 灾害后环境恢复所需回合数
    constexpr double NPP_DISASTER_REDUCTION = 0.3;    // 灾害后NPP降低比例（最高30%）
    constexpr double SOIL_DISASTER_REDUCTION = 1.0;   // 灾害后土壤肥力降低值
    constexpr double DISASTER_RECOVERY_RATE = 0.02;   // 每回合恢复速度（2%）

    // ===== 军队维护与远征消耗常量 =====
    constexpr double ARMY_GOLD_MAINTENANCE = 0.2;     // 每单位军队每回合金币维护费
    constexpr double ARMY_EXTRA_FOOD_COST_PER_DIST = 0.001;  // 远征每距离单位食物额外消耗
    constexpr double ARMY_EXTRA_GOLD_COST_PER_DIST = 0.001;  // 远征每距离单位金币额外消耗

    // ===== 补给站相关常量 =====
    constexpr double SUPPLY_STATION_BUILD_COST = 300.0;      // 建造一个补给站的金币成本
    constexpr double SUPPLY_STATION_MAINTENANCE = 2.0;       // 每个补给站每回合维护费
    constexpr double SUPPLY_STATION_BUILD_COST_INCREASE = 50.0; // 每多建一个补给站，成本增加
    constexpr int    SUPPLY_STATION_MAX_PER_KINGDOM = 20;    // 最大补给站数量
    constexpr double SUPPLY_STATION_AI_BUILD_THRESHOLD_GOLD = 1000.0;  // AI 建造所需金币阈值
    constexpr double SUPPLY_STATION_AI_BUILD_THRESHOLD_ARMY = 200.0;   // AI 建造所需军队阈值
    constexpr double SUPPLY_STATION_AI_BUILD_PROB = 0.02;    // 每回合建造概率
    
        // ===== 驻军支援相关常量 =====
    constexpr double GARRISON_SUPPORT_THRESHOLD = 30.0;      // 低于此驻军值触发支援请求
    constexpr double GARRISON_SOURCE_MIN = 20.0;             // 源定居点至少保留的驻军
    constexpr double SUPPORT_TRAVEL_SPEED = 10.0;            // 每回合移动格数（曼哈顿距离）
    constexpr double SUPPORT_TRANSFER_RATIO = 0.5;           // 转移比例（源驻军的50%）
    constexpr int    SUPPORT_MAX_SEARCH_RADIUS = 100;        // 最大搜索距离（格）
}
...
struct Settlement {
    ...
    double garrison = 0.0;  // 驻军数量（兵力）
    int support_arrival_turn = -1;         // 支援部队预计到达回合（-1表示无支援）
    int support_source_settlement_id = -1; // 支援来源定居点索引（-1表示无）
...
struct Kingdom {
    ...
    std::unordered_set<int> supply_stations;   // 补给站所在格的线性索引
    bool in_world_war = false;

    // 历史峰值数据（用于灭亡后输出）
    int max_territory_size = 0;
    int max_population = 0;
    // 灾害恢复状态：记录每个定居点因灾害导致的NPP/土壤损失，用于逐步恢复
    std::unordered_map<int, double> disaster_npp_penalty;   // key=线性索引, value=当前衰减比例(0~1)
    std::unordered_map<int, double> disaster_soil_penalty;  // key=线性索引, value=当前衰减值
    
    Position get_capital_position() const {
        for (const auto& s : settlements) {
            if (s.is_capital) return s.pos;
        }
        return Position(-1, -1);
    }
...
struct GlobalEvent {
    ...
    std::string name;
    std::string description;
    std::function<bool(const std::vector<Kingdom>&, const WorldData&, int)> condition;  // 全局条件
    std::vector<Effect> effects;   // 对每个存活王国应用
    int global_cooldown;
    int last_triggered_turn;
    bool is_one_time;
    bool triggered_once;
    bool requires_magic;

    GlobalEvent() : id(-1), global_cooldown(0), last_triggered_turn(-1),
                    is_one_time(false), triggered_once(false), requires_magic(false) {}

    GlobalEvent(int _id, const std::string& _name, const std::string& _desc,
            	std::function<bool(const std::vector<Kingdom>&, const WorldData&, int)> _cond,
                const std::vector<Effect>& _eff,
                int _cooldown, bool _one_time = false, bool _magic = false)
        : id(_id), name(_name), description(_desc), condition(_cond), effects(_eff),
          global_cooldown(_cooldown), last_triggered_turn(-1),
          is_one_time(_one_time), triggered_once(false), requires_magic(_magic) {}
};
...
// ===== [新增] 战争状态扩展 =====
struct WarState {
    int start_turn;          // 战争开始回合
    double exhaustion;       // 厌战值 (0~100)
    bool offered_peace;      // 本方是否已提出和谈（用于冷却）
    int peace_offer_turn;    // 提出和谈的回合（-1表示未提出）
    WarState() : start_turn(-1), exhaustion(0.0), offered_peace(false), peace_offer_turn(-1) {}
};
extern std::vector<std::vector<WarState>> warStateMatrix;   // 与 warMatrix 同尺寸，只对 warMatrix[i][j]!=0 有效
...
extern std::vector<GlobalEvent> globalGlobalEvents;
...
void check_surrender(Kingdom& a, Kingdom& b, int turn);
void apply_surrender_diplomatic_penalty(Kingdom& loser, Kingdom& winner, int turn);
...
void process_strong_aid(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn);

void build_roads(Kingdom& k, const WorldData& world, int turn);
void maintain_roads(Kingdom& k, int turn);

double get_nearest_supply_station_distance(const Kingdom& k, int x, int y);
bool build_supply_station(Kingdom& k, const WorldData& world, int x, int y, int turn);
void destroy_supply_station_if_occupied(Kingdom& k, const WorldData& world, int x, int y, int turn);
void maintain_supply_stations(Kingdom& k, int turn);
void ai_build_supply_stations(Kingdom& k, const WorldData& world, int turn);
...
void destroy_buildings_prioritized(Settlement& s, int count);
bool is_settlement_affected_by_disaster(const Settlement& s, const WorldData& world,
                                        const EventTemplate& ev, double& intensity);
void recover_disaster_environment(Kingdom& k, WorldData& world, int turn);
...
void check_global_events(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn);
...
std::vector<std::vector<WarState>> warStateMatrix;
...
std::vector<GlobalEvent> globalGlobalEvents;
...
int create_kingdom(std::vector<Kingdom>& kingdoms, const WorldData& world,
    ...
    // ===== [新增] 初始化战争状态矩阵 =====
	for (auto& row : warStateMatrix) row.resize(new_size, WarState());
	warStateMatrix.resize(new_size, std::vector<WarState>(new_size, WarState()));
...
// ============================================================
// 获取到最近补给站的距离（曼哈顿距离），若无补给站则返回 -1
// ============================================================
double get_nearest_supply_station_distance(const Kingdom& k, int x, int y) {
    if (k.supply_stations.empty()) return -1.0;
    int min_dist = INT_MAX;
    for (int idx : k.supply_stations) {
        int sx = idx % WORLD_WIDTH;
        int sy = idx / WORLD_WIDTH;
...
// ============================================================
// 在指定格建造补给站（前提：该格是本国领土，无定居点，未建补给站，金币足够）
// ============================================================
bool build_supply_station(Kingdom& k, const WorldData& world, int x, int y, int turn) {
    // 检查最大数量限制
    if (k.supply_stations.size() >= Balance::SUPPLY_STATION_MAX_PER_KINGDOM) return false;
    // 检查是否已是补给站
    int idx = linear_index(x, y, WORLD_WIDTH);
    if (k.supply_stations.find(idx) != k.supply_stations.end()) return false;
    // 检查是否是本国领土
    if (ownerMap[y][x] != k.id) return false;
    // 检查该格是否已有定居点
    for (const auto& s : k.settlements) {
        if (s.pos.x == x && s.pos.y == y) return false;
    }
    // 检查是否为陆地且可通行（可放宽）
    if (!is_land(world, x, y)) return false;
    // 计算建造成本（基础 + 数量递增）
    double cost = Balance::SUPPLY_STATION_BUILD_COST +
                  static_cast<double>(k.supply_stations.size()) * Balance::SUPPLY_STATION_BUILD_COST_INCREASE;
    if (k.gold < cost) return false;

    // 扣除金币
    k.gold -= cost;
    // 添加补给站
    k.supply_stations.insert(idx);

    // 记录事件
    std::ostringstream oss;
    oss << k.name << " 在 (" << x << ", " << y << ") 建造了补给站。";
    EventRecord event(turn, format_time(turn), EventType::ROAD, k.id, oss.str(), false, x, y);
    globalEventLog.push_back(event);
    std::cout << "[" << turn << "] " << oss.str() << std::endl;
    return true;
}
...
// ============================================================
// 检查并移除被敌方占领的补给站（调用时机：格子易主时）
// ============================================================
void destroy_supply_station_if_occupied(Kingdom& k, const WorldData& world, int x, int y, int turn) {
    int idx = linear_index(x, y, WORLD_WIDTH);
    if (k.supply_stations.find(idx) == k.supply_stations.end()) return;
    // 若该格不再属于该王国，则移除补给站
    if (ownerMap[y][x] != k.id) {
        k.supply_stations.erase(idx);
        std::ostringstream oss;
        oss << k.name << " 在 (" << x << ", " << y << ") 的补给站被摧毁。";
        EventRecord event(turn, format_time(turn), EventType::WAR, k.id, oss.str(), false, x, y);
        globalEventLog.push_back(event);
        std::cout << "[" << turn << "] " << oss.str() << std::endl;
    }
}
...
// ============================================================
void maintain_supply_stations(Kingdom& k, int turn) {
    if (k.supply_stations.empty()) return;
    double total_maintenance = k.supply_stations.size() * Balance::SUPPLY_STATION_MAINTENANCE;
    if (k.gold >= total_maintenance) {
        k.gold -= total_maintenance;
        return;
    }
    // 金币不足，拆除部分补给站
    // 随机选择一个拆除
    std::vector<int> stations(k.supply_stations.begin(), k.supply_stations.end());
    std::shuffle(stations.begin(), stations.end(), global_rng);
    for (int idx : stations) {
        if (k.gold >= total_maintenance) break; // 已够付
        int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
        k.supply_stations.erase(idx);
        total_maintenance -= Balance::SUPPLY_STATION_MAINTENANCE;
        std::ostringstream oss;
        oss << k.name << " 因资金不足拆除了 (" << x << ", " << y << ") 的补给站。";
        EventRecord event(turn, format_time(turn), EventType::ECONOMIC, k.id, oss.str(), false, x, y);
        globalEventLog.push_back(event);
        std::cout << "[" << turn << "] " << oss.str() << std::endl;
    }
}
...
void ai_build_supply_stations(Kingdom& k, const WorldData& world, int turn) {
    // 检查条件：金币、军队、领土大小等
    if (k.gold < Balance::SUPPLY_STATION_AI_BUILD_THRESHOLD_GOLD) return;
    if (k.army < Balance::SUPPLY_STATION_AI_BUILD_THRESHOLD_ARMY) return;
    if (k.supply_stations.size() >= Balance::SUPPLY_STATION_MAX_PER_KINGDOM) return;
    // 随机概率
    std::uniform_real_distribution<double> prob(0.0, 1.0);
    if (prob(global_rng) > Balance::SUPPLY_STATION_AI_BUILD_PROB) return;

    // 选择候选格：领土内非定居点非补给站且可通行
    std::vector<int> candidates;
    for (int idx : k.territory) {
        if (k.supply_stations.find(idx) != k.supply_stations.end()) continue;
        int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
        // 检查是否已有定居点
        bool has_settlement = false;
        for (const auto& s : k.settlements) {
            if (s.pos.x == x && s.pos.y == y) { has_settlement = true; break; }
        }
        if (has_settlement) continue;
        if (!is_land(world, x, y)) continue;
        candidates.push_back(idx);
    }
    if (candidates.empty()) return;
    // 随机选一个建造
    std::uniform_int_distribution<size_t> dist(0, candidates.size() - 1);
    int idx = candidates[dist(global_rng)];
    int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
    build_supply_station(k, world, x, y, turn);
}
...
void collect_resources(Kingdom& k, WorldData& world, int turn) {
    ...
    for (int idx : k.territory) {
        ...
        // 应用灾害惩罚
        double penalty = 0.0;
        auto it_npp = k.disaster_npp_penalty.find(idx);
        if (it_npp != k.disaster_npp_penalty.end()) {
            penalty += it_npp->second;
        }
        auto it_soil = k.disaster_soil_penalty.find(idx);
        if (it_soil != k.disaster_soil_penalty.end()) {
            // 土壤肥力降低也会影响NPP产出，这里简单将土壤惩罚折半加到npp惩罚上
            penalty += it_soil->second * 0.1; // 0.1为转换系数
        }
        penalty = std::min(1.0, penalty);
        double effective_npp = npp * (1.0 - penalty);
        base_food += effective_npp * Balance::NPP_TO_FOOD_FACTOR * control;
    ...
    for (int idx : k.territory) {
        int x = idx % WORLD_WIDTH;
        int y = idx / WORLD_WIDTH;
        // 获取到最近补给站的距离，若无补给站则返回 -1
        double dist_to_supply = get_nearest_supply_station_distance(k, x, y);
        double dist;
        if (dist_to_supply >= 0) {
            dist = dist_to_supply;  // 使用补给站距离
        } else {
            // 退回到首都距离
            Position capital_pos = k.get_capital_position();
            if (capital_pos.x == -1) continue;
            dist = std::sqrt((x - capital_pos.x)*(x - capital_pos.x) + (y - capital_pos.y)*(y - capital_pos.y));
        }
        total_dist += dist;
        tile_count++;
    }
    if (tile_count > 0) {
        avg_dist = total_dist / tile_count;
    }

    // 计算军队维护费和远征消耗
    double army_gold_cost = k.army * Balance::ARMY_GOLD_MAINTENANCE;
    double extra_food_cost = 0.0;
    double extra_gold_cost = 0.0;
    if (avg_dist > 0) {
        extra_food_cost = k.army * Balance::ARMY_EXTRA_FOOD_COST_PER_DIST * avg_dist;
        extra_gold_cost = k.army * Balance::ARMY_EXTRA_GOLD_COST_PER_DIST * avg_dist;
    }
    ...
    // ---------- 应用所有收入和消耗 ----------
    k.food += total_food - army_food_cost - extra_food_cost - population_food_consumption - food_spoilage;
    k.gold += total_gold - army_gold_cost - extra_gold_cost - building_maintenance - road_maintenance - king_squander - corruption_cost;
...
// 第4部分新增：驻军分配函数
// ============================================================
void distribute_garrison(Kingdom& k) {
    if (k.settlements.empty()) return;
    // 找到首都
    Position capital_pos(-1, -1);
    for (const auto& s : k.settlements) {
        if (s.is_capital) { capital_pos = s.pos; break; }
    }
    if (capital_pos.x == -1) return; // 没有首都（异常）
    ...
    for (const auto& s : k.settlements) {
        double dist = std::sqrt((s.pos.x - capital_pos.x)*(s.pos.x - capital_pos.x) +
                                (s.pos.y - capital_pos.y)*(s.pos.y - capital_pos.y));
        distances.push_back(dist);
        // 权重：距离越近权重越高，且首都额外加成
        double weight = 1.0 - dist * Balance::GARRISON_DISTANCE_DECAY_RATE;
        if (s.is_capital) weight *= Balance::GARRISON_CAPITAL_BONUS;
        weight = std::max(0.1, weight); // 最低权重
        weights.push_back(weight);
        total_weight += weight;
    }

    // 分配军队
    double total_army = k.army;
    double remaining_army = total_army;
    // 先计算每个定居点的初步分配（按权重），然后调整
    std::vector<double> assigned(k.settlements.size(), 0.0);
    for (size_t i = 0; i < k.settlements.size(); ++i) {
        double max_garrison = k.settlements[i].population * Balance::GARRISON_MAX_RATIO;
        double min_garrison = k.settlements[i].population * Balance::GARRISON_MIN_RATIO;
        double raw = (weights[i] / total_weight) * total_army;
        // 初步限制
        assigned[i] = std::max(min_garrison, std::min(max_garrison, raw));
        remaining_army -= assigned[i];
    }

    // 如果还有剩余军队，按比例分配给各定居点（但不超过最大限制）
    if (remaining_army > 0.001) {
        // 计算每个定居点还能容纳多少（最大-当前）
        double total_remaining_capacity = 0.0;
        std::vector<double> remaining_capacity(k.settlements.size(), 0.0);
        for (size_t i = 0; i < k.settlements.size(); ++i) {
            double max_garrison = k.settlements[i].population * Balance::GARRISON_MAX_RATIO;
            double cap = max_garrison - assigned[i];
            if (cap > 0) {
                remaining_capacity[i] = cap;
                total_remaining_capacity += cap;
            }
        }
        if (total_remaining_capacity > 0) {
            for (size_t i = 0; i < k.settlements.size(); ++i) {
                double extra = remaining_army * (remaining_capacity[i] / total_remaining_capacity);
                assigned[i] += extra;
                remaining_army -= extra;
                if (remaining_army < 0.001) break;
            }
        }
    }
    ...
    // 更新每个定居点的驻军
    for (size_t i = 0; i < k.settlements.size(); ++i) {
        k.settlements[i].garrison = assigned[i];
    }
}
...
void update_stability(Kingdom& k, const WorldData& world, int turn) {
    ...
    // ===== 厌战值对稳定度的反馈 =====
    // 计算该王国当前参与的所有战争的厌战值总和
    double total_exhaustion = 0.0;
    int war_count = 0;
    for (size_t j = 0; j < warMatrix.size(); ++j) {
        if (j == static_cast<size_t>(k.id)) continue;
        if (warMatrix[k.id][j] != 0) {
            total_exhaustion += warStateMatrix[k.id][j].exhaustion;
            war_count++;
        }
    }
    if (war_count > 0) {
        double avg_exhaustion = total_exhaustion / war_count;
        // 厌战值每点扣除0.01稳定度，最高每回合扣除2点
        double stab_penalty = std::min(2.0, avg_exhaustion * 0.01);
        adjustment -= stab_penalty;
    }
...
void update_diplomacy(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
        ...
        for (size_t j = i + 1; j < n; ++j) {
            ...
            if (!in_war) {
                ...
                if (prob(global_rng) < p) {
                    warMatrix[i][j] = 1;
                    warMatrix[j][i] = 1;
                    warStateMatrix[i][j].start_turn = turn;
    				warStateMatrix[j][i].start_turn = turn;
    				warStateMatrix[i][j].exhaustion = 0.0;
    				warStateMatrix[j][i].exhaustion = 0.0;
    				warStateMatrix[i][j].offered_peace = false;
    				warStateMatrix[j][i].offered_peace = false;
                ...
                // ===== 只有战争持续至少 5 回合才能考虑和平 =====
				if (turn - warStateMatrix[i][j].start_turn >= 5) {
    				// 计算双方厌战值、军力比、稳定度等
			    	double army_ratio = (a.army + 1.0) / (b.army + 1.0);
			    	double stab_factor = (a.stability + b.stability) / 200.0;   // 0~1
			    	double exhaustion_avg = (warStateMatrix[i][j].exhaustion + warStateMatrix[j][i].exhaustion) / 2.0;
			    	// 综合和平意愿（0~1）
			    	double peace_willingness = 0.3 * (1.0 - army_ratio) + 0.3 * stab_factor + 0.4 * (exhaustion_avg / 100.0);
			    	// 国王性格修正
			    	peace_willingness += 0.1 * (a.diplomacy + b.diplomacy) - 0.1 * (a.aggression + b.aggression) / 2.0;
			    	peace_willingness = std::max(0.0, std::min(1.0, peace_willingness));
	    			// ===== 检查并重置过期的和谈标记（超过5回合自动失效） =====
	    			if (warStateMatrix[i][j].offered_peace && (turn - warStateMatrix[i][j].peace_offer_turn > 5)) {
	    			    warStateMatrix[i][j].offered_peace = false;
	    			    warStateMatrix[i][j].peace_offer_turn = -1;
	    			}
	    			if (warStateMatrix[j][i].offered_peace && (turn - warStateMatrix[j][i].peace_offer_turn > 5)) {
	    			    warStateMatrix[j][i].offered_peace = false;
	    			    warStateMatrix[j][i].peace_offer_turn = -1;
	    			}

			    	// 若一方已提出和谈，增加另一方接受概率
			    	if (warStateMatrix[i][j].offered_peace) peace_willingness += 0.2;
			    	if (warStateMatrix[j][i].offered_peace) peace_willingness += 0.2;

				    // 随机波动
	    			std::uniform_real_distribution<double> noise(0.0, 0.1);
	    			peace_willingness += noise(global_rng) - 0.05;
	
	    			if (prob(global_rng) < peace_willingness) {
	    			    // 达成和平
	   				    warMatrix[i][j] = 0;
	    			    warMatrix[j][i] = 0;
	    			    a.war_timer = 0;
	    			    ...
	    			    b.peace_timer = 40;
	    			    // 清除战争状态
	    			    warStateMatrix[i][j] = WarState();
	    			    warStateMatrix[j][i] = WarState();
	    			    // 原稳定度惩罚等保留
	    			    // ...
	    			} else {
	    			    // 未达成和平，处理“提出和谈”逻辑：若意愿较高但未达成，标记提出和谈（下次对方更容易接受）
	    			    if (peace_willingness > 0.6 && !warStateMatrix[i][j].offered_peace) {
    			        	warStateMatrix[i][j].offered_peace = true;
    			        	warStateMatrix[i][j].peace_offer_turn = turn;
    			    	}
    			    	if (peace_willingness > 0.6 && !warStateMatrix[j][i].offered_peace) {
    			    	    warStateMatrix[j][i].offered_peace = true;
    			    	    warStateMatrix[j][i].peace_offer_turn = turn;
    			    	}
    				}
				}
            }
	...
	for (size_t i = 0; i < n; ++i) {
	    for (size_t j = i + 1; j < n; ++j) {
	        if (warMatrix[i][j] != 0) {
	            check_surrender(kingdoms[i], kingdoms[j], turn);
	        }
	    }
	}
}
...
void check_surrender(Kingdom& a, Kingdom& b, int turn) {
    // ---- 检查 a 是否向 b 投降 ----
    bool a_surrender = false;
    // 条件1：军队极少且稳定度极低
    if (a.army < 5.0 && a.stability < 20.0) a_surrender = true;
    // 条件2：首都沦陷（首都格被 b 占领）
    Position cap_a = a.get_capital_position();
    if (cap_a.x != -1 && ownerMap[cap_a.y][cap_a.x] == b.id) a_surrender = true;
    // 条件3：厌战值超过 90
    if (warStateMatrix[a.id][b.id].exhaustion > 90.0) a_surrender = true;

    if (a_surrender) {
        // ---- 执行投降 ----
        warMatrix[a.id][b.id] = 0;
        warMatrix[b.id][a.id] = 0;
        warStateMatrix[a.id][b.id] = WarState();
        warStateMatrix[b.id][a.id] = WarState();

        // 割让 10% 领土给胜利方（随机选取）
        int tiles_to_lose = static_cast<int>(a.territory.size() * 0.1);
        if (tiles_to_lose > 0) {
            std::vector<int> tiles(a.territory.begin(), a.territory.end());
            std::shuffle(tiles.begin(), tiles.end(), global_rng);
            for (int i = 0; i < tiles_to_lose && i < (int)tiles.size(); ++i) {
                int idx = tiles[i];
                int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
                ownerMap[y][x] = b.id;
                controlMap[y][x] = 0.5;
                cultureMap[y][x] = b.culture;
                a.territory.erase(idx);
                b.territory.insert(idx);
            }
        }
        // 赔款（从 a 转移 100 金币给 b，若不足则全部转移）
        double transfer_gold = std::min(100.0, a.gold);
        a.gold -= transfer_gold;
        b.gold += transfer_gold;
        ...
        // 记录事件
        std::ostringstream oss;
        oss << a.name << " 向 " << b.name << " 投降！割让 " << tiles_to_lose << " 格领土，赔款 " << transfer_gold << " 金币。";
        EventRecord event(turn, format_time(turn), EventType::WAR, a.id, oss.str(), true);
        event.involved_kingdom_ids = {a.id, b.id};
        globalEventLog.push_back(event);
        apply_surrender_diplomatic_penalty(a, b, turn);
        std::cout << "[" << turn << "] " << oss.str() << std::endl;
        return;  // 本对战争已结束，不再检查反向
    }

    // ---- 检查 b 是否向 a 投降（对称逻辑） ----
    bool b_surrender = false;
    if (b.army < 5.0 && b.stability < 20.0) b_surrender = true;
    Position cap_b = b.get_capital_position();
    if (cap_b.x != -1 && ownerMap[cap_b.y][cap_b.x] == a.id) b_surrender = true;
    if (warStateMatrix[b.id][a.id].exhaustion > 90.0) b_surrender = true;
    ...
    if (b_surrender) {
        ...
        int tiles_to_lose = static_cast<int>(b.territory.size() * 0.1);
        if (tiles_to_lose > 0) {
            std::vector<int> tiles(b.territory.begin(), b.territory.end());
            std::shuffle(tiles.begin(), tiles.end(), global_rng);
            for (int i = 0; i < tiles_to_lose && i < (int)tiles.size(); ++i) {
                int idx = tiles[i];
                int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
                ownerMap[y][x] = a.id;
                controlMap[y][x] = 0.5;
                cultureMap[y][x] = a.culture;
                b.territory.erase(idx);
                a.territory.insert(idx);
            }
        }
        double transfer_gold = std::min(100.0, b.gold);
        b.gold -= transfer_gold;
        a.gold += transfer_gold;
        b.stability = std::min(MAX_STABILITY, b.stability + 15.0);

        std::ostringstream oss;
        oss << b.name << " 向 " << a.name << " 投降！割让 " << tiles_to_lose << " 格领土，赔款 " << transfer_gold << " 金币。";
        EventRecord event(turn, format_time(turn), EventType::WAR, b.id, oss.str(), true);
        event.involved_kingdom_ids = {a.id, b.id};
        globalEventLog.push_back(event);
        apply_surrender_diplomatic_penalty(b, a, turn);
        std::cout << "[" << turn << "] " << oss.str() << std::endl;
    }
}

// ============================================================
// 处理到达的支援部队（每回合在王国行动中调用）
// ============================================================
void process_support_arrivals(Kingdom& k, int turn) {
    for (auto& s : k.settlements) {
        // 检查是否有支援在本回合到达
        if (s.support_arrival_turn == turn && s.support_source_settlement_id >= 0) {
            int src_id = s.support_source_settlement_id;
            // 检查源定居点是否仍然存在（索引有效且未被移除）
            if (src_id >= 0 && src_id < static_cast<int>(k.settlements.size())) {
                Settlement& src = k.settlements[src_id];
                // 转移支援兵力（源驻军的一半，但保留至少 GARRISON_SOURCE_MIN）
                double transfer = src.garrison * Balance::SUPPORT_TRANSFER_RATIO;
                // 确保源定居点至少保留最小驻军
                transfer = std::min(transfer, src.garrison - Balance::GARRISON_SOURCE_MIN);
                if (transfer > 0.1) {
                    src.garrison -= transfer;
                    s.garrison += transfer;
                    // 记录事件（可选）
                    std::ostringstream oss;
                    oss << k.name << " 的定居点 " << s.name 
                        << " 收到来自 " << src.name << " 的支援（" 
                        << std::fixed << std::setprecision(0) << transfer << " 兵力）。";
                    EventRecord event(turn, format_time(turn), EventType::WAR, 
                                      k.id, oss.str(), false, s.pos.x, s.pos.y);
                    globalEventLog.push_back(event);
                    std::cout << "[" << turn << "] " << oss.str() << std::endl;
                }
            }
            // 清除支援标记（无论源是否仍存在）
            s.support_arrival_turn = -1;
            s.support_source_settlement_id = -1;
        }
    }
}
...
void process_wars(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
    ...
    for (auto& wp : war_pairs) {
        ...
        // ===== 保存战斗前军队数量（用于计算损失比例） =====
		double initial_atk_army = attacker.army;
		double initial_def_army = defender.army;

        // 获取双方首都位置（用于距离计算）
        Position atk_cap = attacker.get_capital_position();
        Position def_cap = defender.get_capital_position();
        if (atk_cap.x == -1 || def_cap.x == -1) continue; // 无首都则无法作战
        ...
        for (int k = 0; k < max_attacks; ++k) {
            ...
            // ---------- 1. 计算攻击方有效攻击力 ----------
            // 攻击方可调动的军队 = 全国军队 * 部署比例
            double atk_base = attacker.army * Balance::ATTACK_DEPLOYMENT_RATIO;
            // 指挥官影响
            double commander_agg = 0.5;
            if (!attacker.commanders.empty() && attacker.active_cmd_index >= 0 &&
                attacker.active_cmd_index < static_cast<int>(attacker.commanders.size())) {
            ...
            double atk_factor = 0.8 + commander_agg * 0.4;
            double atk_power = atk_base * atk_factor * attacker.combat_modifier() * attacker.cached_military_mod;

            // 距离衰减：距离首都越远，攻击力越低
            double dist_atk = std::sqrt((tx - atk_cap.x)*(tx - atk_cap.x) + (ty - atk_cap.y)*(ty - atk_cap.y));
            double dist_penalty = 1.0 / (1.0 + dist_atk / Balance::DISTANCE_PENALTY_FACTOR);
            atk_power *= dist_penalty;

            // 攻击方建筑加成
            atk_power *= (1.0 + attacker.cached_attack_bonus);

            // 地形攻击修正（基于目标格地形）
            int terrain = static_cast<int>(world.get(LAYER_TERRAIN, tx, ty));
            double terrain_atk_mod = 0.0;
            switch (terrain) {
                case static_cast<int>(TerrainClass::MOUNTAIN):
                case static_cast<int>(TerrainClass::HIGH_MOUNTAIN):
            ...
            atk_power *= (1.0 + terrain_atk_mod);
            atk_power = std::max(1.0, atk_power);

            // ---------- 2. 计算防守方有效防御力 ----------
            // 查找目标格是否属于某个定居点
            double def_power = 0.0;
            int settlement_idx = -1;
            for (size_t si = 0; si < defender.settlements.size(); ++si) {
                const auto& s = defender.settlements[si];
                if (s.pos.x == tx && s.pos.y == ty) {
                    settlement_idx = static_cast<int>(si);
                    // 使用该定居点的驻军
                    def_power = s.garrison;
                    break;
                }
            }
			...
			// ---------- [新增] 驻军支援调度 ----------
			if (settlement_idx != -1) {
			    Settlement& target_sett = defender.settlements[settlement_idx];
			    // 检查该定居点是否已有支援在路上
    			if (target_sett.support_arrival_turn == -1) {
        			// 驻军低于阈值时触发支援请求
        			if (target_sett.garrison < Balance::GARRISON_SUPPORT_THRESHOLD) {
        			    // 寻找最近的、驻军充足的定居点作为支援源
        			    int best_src_idx = -1;
        			    double best_dist = 1e9;
            			for (size_t si = 0; si < defender.settlements.size(); ++si) {
                			if (si == static_cast<size_t>(settlement_idx)) continue;
                			const Settlement& src = defender.settlements[si];
                			// 源定居点必须有足够的驻军
                			if (src.garrison < Balance::GARRISON_SOURCE_MIN + 10.0) continue;
                			// 计算距离（曼哈顿距离）
                			double dist = std::abs(src.pos.x - target_sett.pos.x) 
                			            + std::abs(src.pos.y - target_sett.pos.y);
                			if (dist < best_dist && dist < Balance::SUPPORT_MAX_SEARCH_RADIUS) {
                			    best_dist = dist;
                			    best_src_idx = static_cast<int>(si);
                			}
        			    }
        			    if (best_src_idx != -1) {
        			        // 计算行军回合数（至少1回合）
        			        int travel_turns = std::max(1, static_cast<int>(std::ceil(best_dist / Balance::SUPPORT_TRAVEL_SPEED)));
        			        target_sett.support_arrival_turn = turn + travel_turns;
        			        target_sett.support_source_settlement_id = best_src_idx;
        			        // 可选：记录调度事件（为了减少日志噪音，暂时不记录）
            			}
		        }
		    }
		}
		// ---------- 支援调度结束 ----------
            ...
            if (settlement_idx == -1) {
                // 不是定居点位置，使用全国军队的 10% 作为巡逻力量
                def_power = defender.army * Balance::NON_SETTLEMENT_DEFENSE_RATIO;
            } else {
                // 定居点驻军额外加成：每格驻军给予少量加成（模拟防御工事）
                def_power *= (1.0 + Balance::GARRISON_DEFENSE_BONUS);
            }
            ...
            double def_factor = 1.0 + commander_caution * 0.4;
            def_power *= def_factor * defender.combat_modifier() * defender.cached_military_mod;

            // 防守方距离首都越近，防御加成越高（模拟补给便利）
            double dist_def = std::sqrt((tx - def_cap.x)*(tx - def_cap.x) + (ty - def_cap.y)*(ty - def_cap.y));
            double def_bonus = 1.0 + 0.5 / (1.0 + dist_def / 50.0); // 距离越近加成越大，最大+0.5
            def_power *= def_bonus;
            ...
            int coast = static_cast<int>(world.get(LAYER_COASTTYPE, tx, ty));
            if (coast == static_cast<int>(CoastType::CLIFF) || coast == static_cast<int>(CoastType::FJORD)) {
                terrain_def_mod += 0.3;
            }
            def_power *= (1.0 + terrain_def_mod);

            // 建筑防御加成
            def_power *= (1.0 + defender.cached_defense_bonus);

            def_power = std::max(1.0, def_power);

            // ---------- 3. 战斗判定 ----------
            std::uniform_real_distribution<double> rand_factor(0.9, 1.1);
            double effective_atk = atk_power * rand_factor(global_rng);
            ...
            if (effective_atk > effective_def) {
                ...
                if (new_control <= 0.0) {
                    ...
                    // 检查防守方在该格是否有补给站，若有则移除
                    destroy_supply_station_if_occupied(defender, world, tx, ty, turn);

                    // ---------- 检查该格是否是定居点位置，如果是则移除定居点 ----------
                    if (settlement_idx != -1) {
                        // 移除该定居点
                        defender.settlements.erase(defender.settlements.begin() + settlement_idx);
                        defender.update_cached_effects();
                    }

                    // ---------- 兵力损失（绝对值 + 动态战损比） ----------
                    // 防守方损失较大
                    std::uniform_real_distribution<double> loss_def(80.0, 200.0);
                    std::uniform_real_distribution<double> loss_atk(30.0, 100.0);
                    // 战损比与攻防比相关：优势越大，自身损失越小
                    double ratio = effective_atk / effective_def;
                    double atk_loss_multiplier = 1.0 / (1.0 + ratio);  // 优势大则损失小
                    double def_loss_multiplier = ratio;                // 劣势大则损失大

                    double loss_def_val = loss_def(global_rng) * def_loss_multiplier;
                    double loss_atk_val = loss_atk(global_rng) * atk_loss_multiplier;

                    // 百分比损耗（基于当前兵力）
                    double percent_loss_def = defender.army * 0.02 * def_loss_multiplier;
                    double percent_loss_atk = attacker.army * 0.02 * atk_loss_multiplier;

                    defender.army = std::max(0.0, defender.army - loss_def_val - percent_loss_def);
                    attacker.army = std::max(0.0, attacker.army - loss_atk_val - percent_loss_atk);

                    // 人口损耗（征兵损耗）
                    int pop_loss_def = static_cast<int>(loss_def_val * 0.5 * def_loss_multiplier);
                    int pop_loss_atk = static_cast<int>(loss_atk_val * 0.5 * atk_loss_multiplier);
                    // 防守方人口扣除
                    if (!defender.settlements.empty()) {
                        int total_pop = defender.total_population();
                        if (total_pop > 0) {
                            double ratio_pop = std::min(0.2, static_cast<double>(pop_loss_def) / total_pop);
                    ...
                    if (!attacker.settlements.empty()) {
                        ...
                        if (total_pop > 0) {
                            double ratio_pop = std::min(0.2, static_cast<double>(pop_loss_atk) / total_pop);
                    ...
                    // 战争额外金币消耗（军需）
                    defender.gold = std::max(0.0, defender.gold - 20.0 * def_loss_multiplier);
                    attacker.gold = std::max(0.0, attacker.gold - 20.0 * atk_loss_multiplier);
                ...
                double percent_loss_def = defender.army * 0.01;
                double percent_loss_atk = attacker.army * 0.01;

                attacker.army = std::max(0.0, attacker.army - loss_atk_fail(global_rng) - percent_loss_atk);
                defender.army = std::max(0.0, defender.army - loss_def_fail(global_rng) - percent_loss_def);
		...
		// [新增] 精确计算厌战值增量 (基于实际战损、持续时间、稳定度、性格)
		// 1. 计算本场战争的持续回合数
		int war_duration = turn - warStateMatrix[attacker.id][defender.id].start_turn;
		if (war_duration < 1) war_duration = 1;  // 安全保护

		// 2. 计算本回合双方军队损失比例 (基于战斗前保存的初始值)
		double atk_loss_ratio = (initial_atk_army > 0) ? (1.0 - attacker.army / initial_atk_army) : 0.0;
		double def_loss_ratio = (initial_def_army > 0) ? (1.0 - defender.army / initial_def_army) : 0.0;
		double avg_loss_ratio = (atk_loss_ratio + def_loss_ratio) * 0.5;

		// 3. 计算基础厌战增量 (与持续时间正相关，但增速递减，避免无限膨胀)
		double duration_base = std::log(1.0 + war_duration) * 1.5;  // 对数增长，初期快后期慢

		// 4. 损失惩罚 (损失越大，厌战越高，但设上限)
		double loss_penalty = avg_loss_ratio * 15.0;  // 损失50%军队 → +7.5点
		loss_penalty = std::min(10.0, loss_penalty); // 单回合最高+10点损失惩罚

		// 5. 稳定度修正 (稳定度越低，厌战增加越快)
		double avg_stability = (attacker.stability + defender.stability) * 0.5;
		double stability_modifier = (100.0 - avg_stability) / 100.0 * 3.0;  // 稳定度0时+3，100时+0
		stability_modifier = std::max(0.0, stability_modifier);

		// 6. 国王性格修正 (侵略性高 -> 厌战增长慢；谨慎/外交高 -> 厌战增长快)
		double avg_aggression = (attacker.aggression + defender.aggression) * 0.5;
		double personality_modifier = (0.5 - avg_aggression) * 4.0;  // 侵略1.0时 → -2.0；侵略0.0时 → +2.0
		personality_modifier = std::max(-2.0, std::min(2.0, personality_modifier));

		// 7. 微小随机扰动 (±5%)，增加不确定性但不过分
		std::uniform_real_distribution<double> noise(-0.05, 0.05);
		double noise_factor = 1.0 + noise(global_rng);

		// 8. 合计本回合厌战增量 (确保至少0.5，最多10)
		double delta_exhaustion = (duration_base + loss_penalty + stability_modifier + personality_modifier) * noise_factor;
		delta_exhaustion = std::max(0.5, std::min(10.0, delta_exhaustion));

		// 9. 分别累加到攻守双方的厌战值上 (双方感受相同，但可分别累积，这里先保持一致)
		warStateMatrix[attacker.id][defender.id].exhaustion += delta_exhaustion;
		warStateMatrix[defender.id][attacker.id].exhaustion += delta_exhaustion;

		// 10. 截断到 [0, 100] 区间
		warStateMatrix[attacker.id][defender.id].exhaustion = std::min(100.0, warStateMatrix[attacker.id][defender.id].exhaustion);
		warStateMatrix[defender.id][attacker.id].exhaustion = std::min(100.0, warStateMatrix[defender.id][attacker.id].exhaustion);
...
void check_world_war(std::vector<Kingdom>& kingdoms, int turn) {
    ...
    // ===== [新增] 检查世界大战的持续时间条件 =====
	constexpr int MIN_WAR_TURNS_FOR_WORLD_WAR = 3;  // 可调
	bool war_duration_ok = true;
	for (int pid : participants) {
    	// 检查该参战国与其他所有参战国之间的战争是否都已持续足够回合
    	for (int other : participants) {
    	    if (pid == other) continue;
    	    if (warMatrix[pid][other] != 0) {
    	        int start = warStateMatrix[pid][other].start_turn;
    	        if (start == -1 || (turn - start) < MIN_WAR_TURNS_FOR_WORLD_WAR) {
    	            war_duration_ok = false;
    	            break;
    	        }
    	    }
    	}
    	if (!war_duration_ok) break;
	}

	// 若持续时间不足，则不清除已有的大战标记，但也不触发新的大战事件
	if (!war_duration_ok) {
	    // 可以选择记录一条“大规模冲突”但非世界大战的信息（可选）
	    // 这里直接返回，不继续触发世界大战事件
	    return;
	}
...
// ============================================================
// 强国援助处理（国王外交属性高且资源丰富时主动援助弱国）
// ============================================================
void process_strong_aid(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
    const size_t n = kingdoms.size();
    if (n < 2) return;

    // 每王国每回合最多援助一次
    std::unordered_set<int> aided_this_turn;

    // 随机打乱顺序，让不同的王国有机会先行动
    std::vector<int> indices;
    for (size_t i = 0; i < n; ++i) {
        if (kingdoms[i].alive) indices.push_back(static_cast<int>(i));
    }
    std::shuffle(indices.begin(), indices.end(), global_rng);

    for (int aid_id : indices) {
        Kingdom& donor = kingdoms[aid_id];
        if (!donor.alive) continue;
        // 条件：外交属性 > 0.7，且资源充足
        if (donor.diplomacy <= 0.7) continue;
        if (donor.gold < 2000.0 || donor.food < 2000.0) continue;
        // 本回合已援助过？跳过
        if (aided_this_turn.count(donor.id)) continue;

        // 寻找合适的受援国：稳定度 < 40 或处于战争状态，且与捐助国接壤或文化相似
        std::vector<int> candidates;
        for (size_t j = 0; j < n; ++j) {
            if (j == static_cast<size_t>(donor.id)) continue;
            Kingdom& recipient = kingdoms[j];
            if (!recipient.alive) continue;
            // 受援国必须满足条件：稳定度低或正在战争
            if (recipient.stability >= 40.0 && recipient.war_timer == 0) continue;
            // 检查是否接壤（简单边界检查）
            if (!kingdoms_share_border(donor, recipient)) continue;
            // 文化相似度（可放宽）
            double sim = cosine_similarity(donor.culture, recipient.culture);
            if (sim < 0.3) continue; // 文化差异太大，不援助
        ...
        // 随机选择一个受援国
        std::uniform_int_distribution<size_t> dist(0, candidates.size() - 1);
        int rec_id = candidates[dist(global_rng)];
        Kingdom& recipient = kingdoms[rec_id];

        // 执行援助：捐助方支付，受援方获得
        double gold_give = 200.0;
        double food_give = 100.0;
        // 检查捐助方是否能支付（可能有其他消耗，再检查一次）
        if (donor.gold < gold_give || donor.food < food_give) continue;

        donor.gold -= gold_give;
        donor.food -= food_give;
        recipient.gold += 100.0;
        recipient.food += 50.0;
        recipient.stability = std::min(MAX_STABILITY, recipient.stability + 10.0);

        // 记录事件
        std::ostringstream oss;
        oss << donor.name << " 向 " << recipient.name << " 提供援助（金币" << gold_give
            << "，食物" << food_give << "），" << recipient.name << " 稳定度提升。";
        EventRecord event(turn, format_time(turn), EventType::INTERNATIONAL,
                          donor.id, oss.str(), true);
        event.involved_kingdom_ids = {donor.id, rec_id};
        globalEventLog.push_back(event);
        std::cout << "[" << turn << "] " << oss.str() << std::endl;

        // 标记本回合已援助
        aided_this_turn.insert(donor.id);
    }
}

// ============================================================
// 投降后外交关系惩罚（预留接口）
// 未来实现外交矩阵时，在此添加关系降低逻辑
// ============================================================
void apply_surrender_diplomatic_penalty(Kingdom& loser, Kingdom& winner, int turn) {
    // TODO: 当实现外交关系矩阵时，在此处降低 loser 与 winner 的关系值
    // 例如：diplomacyMatrix[loser.id][winner.id] -= 50;
    //       diplomacyMatrix[winner.id][loser.id] -= 50;
    // 并记录事件（可选）
    // 目前留空，仅用于标记
    (void)loser;
    (void)winner;
...
// ============================================================
void init_global_events() {
    globalGlobalEvents.clear();

    // 1. 黑死病大瘟疫
    // 条件：至少存在 5 个王国，全球平均人口 > 200，世界总回合 > 100，且至少一个王国稳定度 < 40
    globalGlobalEvents.emplace_back(
        1001, "黑死病大瘟疫",
        "一场致命的瘟疫席卷全球，各国人口锐减，经济崩溃。",
        [](const std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) -> bool {
            ...
            for (const auto& k : kingdoms) {
                if (!k.alive) continue;
                alive++;
                total_pop += k.total_population();
                if (k.stability < 40.0) unstable++;
            }
            if (alive < 5) return false;
            double avg_pop = static_cast<double>(total_pop) / alive;
            if (avg_pop < 200.0) return false;
            if (unstable < 1) return false;
            return true;
        },
        std::vector<Effect>{
            {EffectStat::POPULATION, -0.15},   // 人口减少15%
            {EffectStat::GOLD, -200.0},
            {EffectStat::STABILITY, -15.0},
            {EffectStat::FOOD, -100.0}
        },
        100,   // 全局冷却100回合
        true,  // 仅触发一次
        false  // 不依赖魔法
    ...
    // 2. 黄金时代
    // 条件：存在至少3个王国，全球平均科技 > 5，平均稳定度 > 70，且至少一个王国文明等级 >= 5
    globalGlobalEvents.emplace_back(
        1002, "全球黄金时代",
        "世界进入前所未有的繁荣时期，科技、文化、经济全面腾飞。",
        [](const std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) -> bool {
            if (turn < 50) return false;
            int alive = 0;
            double sum_tech = 0.0;
            double sum_stab = 0.0;
            int high_civ = 0;
            for (const auto& k : kingdoms) {
                if (!k.alive) continue;
                alive++;
                sum_tech += k.tech_level;
                sum_stab += k.stability;
                if (k.civilization_level >= 5) high_civ++;
            }
            if (alive < 3) return false;
            if (sum_tech / alive < 5.0) return false;
            if (sum_stab / alive < 70.0) return false;
            if (high_civ < 1) return false;
            return true;
        },
        std::vector<Effect>{
            {EffectStat::GOLD, 300.0},
            {EffectStat::STABILITY, 10.0},
            {EffectStat::RESEARCH_POINTS, 0.3},
            {EffectStat::FOOD, 200.0}
        },
        80,
        true,
        false
    ...
    // 3. 世界战争浪潮
    // 条件：存在至少4个王国，至少有两对战争，且总战争数 >= 3，平均稳定度 < 50
    globalGlobalEvents.emplace_back(
        1003, "世界战争浪潮",
        "多国陷入战争泥潭，全球陷入动荡。",
        [](const std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) -> bool {
            if (turn < 30) return false;
            int alive = 0;
            int war_pairs = 0;
            double sum_stab = 0.0;
            for (const auto& k : kingdoms) {
                ...
                for (const auto& other : kingdoms) {
                    if (!other.alive || other.id <= k.id) continue;
                    if (warMatrix[k.id][other.id] != 0) war_pairs++;
                }
            }
            if (alive < 4) return false;
            if (war_pairs < 2) return false;
            if (sum_stab / alive > 50.0) return false;
            return true;
        },
        std::vector<Effect>{
            {EffectStat::STABILITY, -10.0},
            {EffectStat::GOLD, -100.0},
            {EffectStat::FOOD, -50.0},
            {EffectStat::ARMY, -20.0}
        },
        60,
        true,
        false
    ...
    // 4. 宗教大复兴（依赖魔法）
    // 条件：有至少 2 个王国拥有魔法科技，且平均稳定度 < 50
    globalGlobalEvents.emplace_back(
        1004, "宗教大复兴",
        "神秘力量唤醒世人信仰，稳定度大幅提升。",
        [](const std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) -> bool {
            if (!FeatureToggle::MAGIC) return false;
            if (turn < 40) return false;
            int alive = 0;
            int magic_kingdoms = 0;
            double sum_stab = 0.0;
            for (const auto& k : kingdoms) {
                if (!k.alive) continue;
                alive++;
                ...
                for (TechID tid : k.owned_techs) {
                    if (tid == TechID::MAGIC_BASICS || tid == TechID::ARCANE_RESEARCH) {
                        has_magic = true; break;
                    }
                }
                if (has_magic) magic_kingdoms++;
            }
            if (alive < 3) return false;
            if (magic_kingdoms < 2) return false;
            if (sum_stab / alive > 50.0) return false;
            return true;
        },
        std::vector<Effect>{
            {EffectStat::STABILITY, 20.0},
            {EffectStat::MAGIC_AFFINITY, 0.2},
            {EffectStat::CULTURE_CHANGE_MOD, 0.05}
        },
        50,
        false,   // 可重复触发
        true
    ...
    // 5. 大规模移民潮
    // 条件：至少有 3 个王国，其中一个领土过大（>300格）且人口密度低（<0.5人/格），另一个人口密度高（>2人/格）
    globalGlobalEvents.emplace_back(
        1005, "大规模移民潮",
        "人口从拥挤地区涌向地广人稀之地，改变世界人口分布。",
        [](const std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) -> bool {
            ...
            for (const auto& k : kingdoms) {
                if (!k.alive) continue;
                alive++;
                double area = static_cast<double>(k.territory.size());
                if (area < 10) continue;
                double pop = static_cast<double>(k.total_population());
                double density = pop / area;
                if (area > 300 && density < 0.5) has_sparse = true;
                if (density > 2.0) has_dense = true;
            }
            if (alive < 3) return false;
            if (!has_sparse || !has_dense) return false;
            return true;
        },
        std::vector<Effect>{
            {EffectStat::POPULATION, 0.05},   // 每个王国人口增长5%
            {EffectStat::STABILITY, -5.0},
            {EffectStat::FOOD, -30.0}
        },
        40,
        false,
        false
...
void check_global_events(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
    ...
    if (current_month != 0) return;  // 仅在每年1月检查

    for (auto& ge : globalGlobalEvents) {
        // 检查是否处于冷却期
        if (ge.global_cooldown > 0 && ge.last_triggered_turn != -1 &&
            (turn - ge.last_triggered_turn) < ge.global_cooldown) {
            continue;
        }
        // 若仅触发一次且已触发，跳过
        if (ge.is_one_time && ge.triggered_once) continue;
        // 若需要魔法但未启用，跳过
        if (ge.requires_magic && !FeatureToggle::MAGIC) continue;

        // 执行条件检查
        if (!ge.condition(kingdoms, world, turn)) continue;

        // 条件满足，触发事件
        ge.last_triggered_turn = turn;
        ge.triggered_once = true;

        // 对每个存活王国应用效果
        for (auto& k : kingdoms) {
            if (!k.alive) continue;
            apply_effects(k, ge.effects);
        }

        // 记录事件
        std::string desc = ge.description;
        // 可以替换占位符，这里不扩展
        EventRecord event(turn, format_time(turn), EventType::INTERNATIONAL, -1, desc, true);
        globalEventLog.push_back(event);
        std::cout << "[" << turn << "] 全球事件触发: " << ge.name << " - " << desc << std::endl;

        // 若事件仅触发一次，或设置了冷却，则不再检查其他事件（可让多个事件同时触发，但为控制节奏，每次只触发一个）
        // 这里我们每次只触发一个事件（避免多事件同时爆发），按优先级可调整顺序
        break;  // 每次只触发一个
    }
}
// ---------- 国内显性事件处理 ----------
...
bool is_settlement_affected_by_disaster(const Settlement& s, const WorldData& world,
    ...
    if (ev.name == "火山爆发") {
        // 检查火山活动强度（LAYER_VOLCANIC 值 0~2）
        double vol = world.get(LAYER_VOLCANIC, x, y);
        if (vol > 0.0) {
            intensity = vol / 2.0;  // 归一化到0~1
            return true;
        }
        // 也可检查周围一定半径内是否有火山（简化略）
        return false;
    }
    else if (ev.name == "洪水") {
        double elev = world.get(LAYER_ELEVATION, x, y);
        double precip = world.get(LAYER_PRECIP, x, y);
        // 海拔<10且降水>1000 或 海拔<5且降水>800 等
        if (elev < 10.0 && precip > 1000.0) {
            intensity = std::min(1.0, (10.0 - elev) / 10.0 * 0.5 + (precip - 1000.0) / 1000.0 * 0.5);
            return true;
        }
        if (elev < 5.0 && precip > 800.0) {
            intensity = std::min(1.0, (5.0 - elev) / 5.0 * 0.5 + (precip - 800.0) / 800.0 * 0.5);
            return true;
        }
        return false;
    }
    else if (ev.name == "干旱") {
        double aridity = world.get(LAYER_ARIDITY, x, y);
        double precip = world.get(LAYER_PRECIP, x, y);
        // 干旱指数>2 或 降水<300
        if (aridity > 2.0 || precip < 300.0) {
            intensity = std::min(1.0, (aridity - 2.0) / 3.0 * 0.7 + (300.0 - precip) / 300.0 * 0.3);
            return true;
        }
        return false;
    }
    else if (ev.name == "地震") {
        // 地震受基岩类型影响：沉积岩和火山岩更易受灾
        int bedrock = static_cast<int>(world.get(LAYER_BEDROCK, x, y));
        double base_prob = 0.0;
        if (bedrock == static_cast<int>(BedrockType::SEDIMENTARY)) base_prob = 0.8;
        else if (bedrock == static_cast<int>(BedrockType::VOLCANIC)) base_prob = 0.7;
        else if (bedrock == static_cast<int>(BedrockType::METAMORPHIC)) base_prob = 0.4;
        else base_prob = 0.3;
        // 结合地形坡度修正
        double slope = world.get(LAYER_SLOPE, x, y);
        if (slope > 0.5) base_prob += 0.2;
        intensity = std::min(1.0, base_prob);
        return (intensity > 0.3);  // 只有强度>0.3才视为受灾
    }
    else if (ev.name == "瘟疫") {
        // 瘟疫与人口密度相关（定居点人口/该格承载力？简化使用人口数）
        int pop = s.population;
        if (pop > 100) {
            intensity = std::min(1.0, (pop - 100) / 400.0 * 0.8 + 0.2);
            return true;
        }
        return false;
    }
        ...
        double elev = world.get(LAYER_ELEVATION, x, y);
        int biome = static_cast<int>(world.get(LAYER_BIOME, x, y));
        bool cold_biome = (biome == static_cast<int>(Biome::SNOW_PLAINS) ||
                           biome == static_cast<int>(Biome::SNOW_FOREST) ||
                           biome == static_cast<int>(Biome::TUNDRA) ||
                           biome == static_cast<int>(Biome::ICE_CAP) ||
                           biome == static_cast<int>(Biome::GLACIER));
        if (elev > 100.0 || cold_biome) {
            intensity = std::min(1.0, (elev - 100.0) / 200.0 * 0.6 + (cold_biome ? 0.4 : 0.0));
            return true;
        }
        return false;
    }
...
// ============================================================
void recover_disaster_environment(Kingdom& k, WorldData& world, int turn) {
    for (auto it = k.disaster_npp_penalty.begin(); it != k.disaster_npp_penalty.end(); ) {
        it->second -= Balance::DISASTER_RECOVERY_RATE;
        if (it->second < 0.0) it->second = 0.0;
        // 恢复实际NPP（需要world的引用）
        int idx = it->first;
        int x = idx % WORLD_WIDTH, y = idx / WORLD_WIDTH;
        // 获取原始NPP（需要存储原始值，但我们无法恢复，简化：直接加回）
        // 更好的方式：在灾害时记录原始NPP，但为了简单，我们采用递增加回
        // 由于没有保存原始值，我们只能假设恢复速率与惩罚成正比，逐步加回
        // 注意：可能world的NPP已经被修改，这里我们采用另一种方式：在灾害时降低NPP，恢复时逐步增加
        // 本实现不直接修改NPP，而是修改一个衰减因子，在资源采集时应用。
        // 但为了简单，我们直接在资源采集时检查这些penalty，并降低产出。
        // 因此这里只更新penalty值，实际恢复在collect_resources中体现。
        if (it->second == 0.0) {
            it = k.disaster_npp_penalty.erase(it);
        } else {
            ++it;
        }
    }
    // 同理处理土壤
    for (auto it = k.disaster_soil_penalty.begin(); it != k.disaster_soil_penalty.end(); ) {
        it->second -= Balance::DISASTER_RECOVERY_RATE * 0.5; // 土壤恢复慢一些
        if (it->second < 0.0) it->second = 0.0;
        if (it->second == 0.0) {
            it = k.disaster_soil_penalty.erase(it);
        } else {
            ++it;
        }
    }
...
// ============================================================
// 摧毁定居点中的建筑（按类别优先摧毁脆弱建筑）
// ============================================================
void destroy_buildings_prioritized(Settlement& s, int count) {
    if (s.buildings.empty() || count <= 0) return;
    // 定义脆弱建筑类别（优先摧毁）
    std::vector<BuildingType> fragile = {
        BuildingType::FARM,
        BuildingType::GRANARY,
        BuildingType::IRRIGATION_DITCH,
    ...
    std::vector<BuildingType> normal = {
        BuildingType::MARKET,
        BuildingType::MARKET_SQUARE,
        ...
        BuildingType::SAGE_HALL,
        BuildingType::CATHEDRAL,
        BuildingType::THEATER,
        BuildingType::ARENA,
        BuildingType::HOSPITAL,
        BuildingType::ORPHANAGE,
        ...
        BuildingType::ARCANE_LAB,
        BuildingType::MANA_WELL,
        BuildingType::ENCHANTED_TOWER,
        BuildingType::MINE,
        // 种族特色建筑略...
    };
    // 分类当前建筑
    std::vector<BuildingType> fragile_present, normal_present;
    for (BuildingType bt : s.buildings) {
        if (std::find(fragile.begin(), fragile.end(), bt) != fragile.end())
            fragile_present.push_back(bt);
        else
            normal_present.push_back(bt);
    }
    // 先摧毁脆弱建筑
    std::shuffle(fragile_present.begin(), fragile_present.end(), global_rng);
    int destroyed = 0;
    for (BuildingType bt : fragile_present) {
        if (destroyed >= count) break;
        auto it = std::find(s.buildings.begin(), s.buildings.end(), bt);
        if (it != s.buildings.end()) {
            s.buildings.erase(it);
            destroyed++;
        }
    }
    // 若还需摧毁，从普通建筑中随机删
    if (destroyed < count) {
        std::shuffle(normal_present.begin(), normal_present.end(), global_rng);
        for (BuildingType bt : normal_present) {
            if (destroyed >= count) break;
            auto it = std::find(s.buildings.begin(), s.buildings.end(), bt);
            if (it != s.buildings.end()) {
                s.buildings.erase(it);
...
void apply_disasters(Kingdom& k, const WorldData& world, int turn) {
    ...
    for (EventTemplate& ev : globalEventTemplates) {
        ...
        // 收集受影响的定居点及其强度
        struct AffectedSettlement {
            int index;
            double intensity;
        };
        std::vector<AffectedSettlement> affected;
        for (size_t i = 0; i < k.settlements.size(); ++i) {
            double intensity = 0.0;
            if (is_settlement_affected_by_disaster(k.settlements[i], world, ev, intensity)) {
                affected.push_back({static_cast<int>(i), intensity});
            }
        }
        if (affected.empty()) continue;

        // 触发判定（使用模板基础概率，可结合强度调整）
        double trigger_prob = ev.probability;
        // 若平均强度较高，增加触发概率（可选）
        double avg_intensity = 0.0;
        for (auto& a : affected) avg_intensity += a.intensity;
        avg_intensity /= affected.size();
        trigger_prob *= (0.8 + 0.4 * avg_intensity); // 强度高则更易触发
        trigger_prob = std::min(1.0, trigger_prob);

        if (prob(global_rng) >= trigger_prob) continue;

        // ---- 灾害触发 ----
        mark_event_triggered(ev.id, k.id, turn);

        // 从模板中提取基础效果值（负值表示损失）
        double base_pop_loss_ratio = 0.0;
        double base_gold_loss = 0.0;
        double base_stab_loss = 0.0;
        for (const Effect& eff : ev.effects) {
            if (eff.stat == EffectStat::POPULATION) base_pop_loss_ratio = eff.value;
            else if (eff.stat == EffectStat::GOLD) base_gold_loss = eff.value;
            else if (eff.stat == EffectStat::STABILITY) base_stab_loss = eff.value;
        }

        // 对每个受影响的定居点应用效果（强度加权）
        for (auto& a : affected) {
            Settlement& s = k.settlements[a.index];
            double intensity = a.intensity;
            // 人口损失（按比例，强度越大损失越大）
            if (base_pop_loss_ratio < 0.0) {
                double loss_ratio = (-base_pop_loss_ratio) * (0.5 + 0.5 * intensity);
                int reduction = static_cast<int>(std::llround(s.population * loss_ratio));
                s.population = std::max(Balance::MIN_SETTLEMENT_POP, s.population - reduction);
            }
            // 金币损失（按强度加权，且根据定居点人口规模调整）
            if (base_gold_loss < 0.0) {
                double gold_multiplier = 0.5 + 0.5 * intensity;
                // 根据定居点人口调整金币损失（大城损失更多）
                double pop_factor = std::min(1.0, s.population / 100.0);
                double loss = (-base_gold_loss) * gold_multiplier * (0.5 + 0.5 * pop_factor);
                k.gold = std::max(0.0, k.gold - loss);
            }
            // 稳定度损失
            if (base_stab_loss < 0.0) {
                double stab_loss = (-base_stab_loss) * (0.5 + 0.5 * intensity);
                k.stability = std::max(MIN_STABILITY, k.stability - stab_loss);
            }

            // 建筑摧毁：1~2个，强度高时摧毁更多
            int destroy_count = 1 + static_cast<int>(intensity * 2); // 1~3个
            destroy_count = std::min(destroy_count, static_cast<int>(s.buildings.size()));
            if (destroy_count > 0) {
                destroy_buildings_prioritized(s, destroy_count);
            }

            // 环境后效：降低NPP和土壤肥力（记录惩罚）
            int idx = linear_index(s.pos.x, s.pos.y, WORLD_WIDTH);
            double npp_penalty = intensity * Balance::NPP_DISASTER_REDUCTION;
            double soil_penalty = intensity * Balance::SOIL_DISASTER_REDUCTION;
            if (npp_penalty > 0.01) {
                // 累加惩罚（多次灾害可叠加）
                k.disaster_npp_penalty[idx] = std::min(1.0, k.disaster_npp_penalty[idx] + npp_penalty);
            }
            if (soil_penalty > 0.01) {
                k.disaster_soil_penalty[idx] = std::min(5.0, k.disaster_soil_penalty[idx] + soil_penalty);
            }
        }
        ...
        size_t pos = desc.find("{kingdom}");
        if (pos != std::string::npos) {
            desc.replace(pos, 9, k.name);
            desc += "（影响 " + std::to_string(affected.size()) + " 个定居点，最大强度 " +
                    std::to_string(affected.front().intensity) + "）";
        }
        EventRecord event(turn, format_time(turn),
                          ev.type, k.id, desc, false);
        globalEventLog.push_back(event);
...
void run_simulation(const std::string& world_file, const std::string& output_dir) {
    ...
    for (int turn = 0; turn < MAX_TURNS; ++turn) {
        ...
        for (int kid : order) {
            ...
            k.update_cached_effects();
            distribute_garrison(k);   // 新增
            process_support_arrivals(k, turn);
            collect_resources(k, world, turn);
            recover_disaster_environment(k, world, turn);   // 新增
            ...
            maintain_supply_stations(k, turn);
            ai_build_supply_stations(k, world, turn);
        ...
        // ===== [新增] 强国援助处理 =====
        process_strong_aid(kingdoms, world, turn);
        ...
        if (current_month == 0) {
            ...
            // ===== [新增] 检查全球事件（条件触发） =====
    		check_global_events(kingdoms, world, turn);
        }
