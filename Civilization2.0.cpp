...
constexpr double ROAD_MAINTENANCE_PER_TILE   = 0.2;
...
constexpr double ARMY_FOOD_CONSUMPTION       = 0.2;
...
constexpr double RESEARCH_BASE_POINTS  = 0.05;
constexpr double TECH_DISCOVERY_BASE   = 0.05;
...
constexpr double ROAD_MAINTENANCE      = 0.2;
constexpr double ROAD_COST_BASE        = 5.0;
constexpr double ROAD_COST_SLOPE       = 20.0;
constexpr double ROAD_ASTAR_SLOPE      = 10.0;
...
constexpr double NEW_SETTLEMENT_SCORE_THRESH = 15.0;
...
std::string generate_settlement_name(Race race, bool is_capital) {
    static const std::vector<std::string> prefix_human = {"新", "旧", "北", "南", "东", "西"};
    static const std::vector<std::string> prefix_dwarf = {"石", "铁", "金", "铜", "深"};
    static const std::vector<std::string> prefix_elf = {"翠", "银", "月", "星", "林"};
    static const std::vector<std::string> prefix_orc = {"血", "骨", "战", "暗", "炎"};
    static const std::vector<std::string> prefix_halfling = {"绿", "丘", "河", "花", "蜜"};
    static const std::vector<std::string> suffix = {"城", "堡", "镇", "村", "港", "要塞"};
    ...
    std::string name = (*prefixes)[dist_p(global_rng)] + suffix[dist_s(global_rng)];
    if (is_capital) name = "首都" + name;
    return name;
}
...
std::vector<BuildingData> create_building_data() {
    ...
    { Effect(EffectStat::GOLD, 6.0) });
    add_common(BuildingType::MARKET_SQUARE, "市场广场", "economy", 50.0, 0.5, 1,
    { Effect(EffectStat::GOLD, 8.0), Effect(EffectStat::TRADE_MOD, 0.05) });
    add_common(BuildingType::TRADING_POST, "商栈", "economy", 45.0, 0.4, 1,
    { Effect(EffectStat::GOLD, 5.0), Effect(EffectStat::TRADE_MOD, 0.10) });
    add_common(BuildingType::MINT, "铸币厂", "economy", 60.0, 0.6, 1,
    { Effect(EffectStat::GOLD, 5.0), Effect(EffectStat::TAX_MOD, 0.10) });
    add_common(BuildingType::GUILD_HALL, "商会", "economy", 80.0, 0.8, 1,
    {   Effect(EffectStat::GOLD, 8.0), Effect(EffectStat::TRADE_MOD, 0.15),
        Effect(EffectStat::STABILITY, 1.0)
    });
...
void manage_settlements(Kingdom& k, const WorldData& world, int turn) {
    ...
    for (const BuildCandidate& cand : candidates) {
        ...
        if (!bd_ptr) continue;
        double cost = bd_ptr->cost * k.cached_building_cost_mod;
        if (k.gold < cost) continue;
        k.gold -= cost;
        k.settlements[si].buildings.push_back(type);
        buildings_built++;
    }
...
void update_stability(Kingdom& k, const WorldData& world, int turn) {
    ...
    // 8. 叛乱风险修正（降低叛乱风险相当于提升稳定度）
    // cached_rebellion_risk_mod 通常为1.0，若小于1则增加稳定度
        ...
        for (int k = 0; k < max_attacks; ++k) {
            ...
            if (effective_atk > effective_def) {
                ...
                if (new_control <= 0.0) {
                    ...
                    defender.army = std::max(0.0, defender.army - loss_def(global_rng));
                    attacker.army = std::max(0.0, attacker.army - loss_atk(global_rng));
                } else {
