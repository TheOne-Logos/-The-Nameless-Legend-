...
constexpr double BASE_POP_GROWTH_RATE  = 0.03;
...
constexpr double KING_DEATH_AGE_PER_YEAR = 0.01;
constexpr double KING_DEATH_WAR_BONUS  = 0.03;
}
...
std::string generate_settlement_name(Race race, bool is_capital) {
    // 扩充前缀池
    static const std::vector<std::string> prefix_human = {"新", "旧", "北", "南", "东", "西", "中", "上", "下", "大", "小", "白", "黑", "红", "金", "银", "铁", "铜", "石", "木", "水", "火", "风", "云", "星", "月", "日", "光", "明", "暗", "深", "浅", "高", "低", "远", "近", "内", "外", "前", "后"};
    static const std::vector<std::string> prefix_dwarf = {"石", "铁", "金", "铜", "银", "深", "熔", "锻", "锤", "砧", "矿", "岩", "晶", "钢", "黑", "赤", "灰", "白", "山", "岭", "谷", "渊", "炉", "堡", "卫", "坚", "固", "厚", "硬", "锐"};
    static const std::vector<std::string> prefix_elf = {"翠", "银", "月", "星", "林", "叶", "花", "露", "晨", "暮", "风", "云", "虹", "白", "金", "晶", "灵", "树", "藤", "泉", "湖", "溪", "谷", "丘", "原", "森", "幽", "明", "辉", "光"};
    static const std::vector<std::string> prefix_orc = {"血", "骨", "战", "暗", "炎", "刃", "裂", "碎", "狂", "暴", "铁", "黑", "赤", "灰", "怒", "吼", "锋", "利", "坚", "石", "岩", "渊", "坑", "疤", "伤", "影", "噬", "魂", "祭", "牲"};
    static const std::vector<std::string> prefix_halfling = {"绿", "丘", "河", "花", "蜜", "甜", "暖", "阳", "春", "秋", "丰", "收", "果", "园", "田", "穗", "穗", "麦", "谷", "溪", "桥", "磨", "坊", "仓", "窖", "炉", "炊", "香", "醇", "悠"};

    // 扩充后缀池
    static const std::vector<std::string> suffix = {"城", "堡", "镇", "村", "港", "要塞", "渡", "关", "驿", "亭", "台", "阁", "楼", "殿", "宫", "府", "邸", "庄", "园", "圃", "苑", "林", "野", "原", "洲", "岛", "湾", "峡", "口", "门", "津", "渡", "泊", "泽", "池", "塘", "渊", "涧", "泉", "溪", "河", "江", "湖", "海", "山", "岭", "峰", "崖", "谷", "壑"};

    const std::vector<std::string>* prefixes = nullptr;
    switch (race) {
    case Race::HUMAN:
        prefixes = &prefix_human;
        break;
    case Race::DWARF:
        prefixes = &prefix_dwarf;
        break;
    case Race::ELF:
        prefixes = &prefix_elf;
        break;
    ...
    // 可能使用双前缀
    std::string name;
    if (std::uniform_real_distribution<double>(0.0, 1.0)(global_rng) < 0.3) {
        // 双前缀
    ...
    if (is_capital) {
        // 首都特殊名称：在名称前加“首都”或特殊修饰
        std::vector<std::string> capital_prefix = {"首都", "王都", "京都", "皇城", "圣城", "大城"};
...
std::string generate_king_name(Race race) {
    static const std::vector<std::string> human_names = {"阿尔弗雷德", "爱德华", "威廉", "亨利", "理查德", "罗伯特", "威廉明娜", "伊莎贝拉"};
    static const std::vector<std::string> dwarf_names = {"托林", "巴林", "德瓦林", "欧因", "格罗因", "金雳", "索林"};
    static const std::vector<std::string> elf_names = {"莱戈拉斯", "瑟兰迪尔", "埃尔德隆", "凯兰崔尔", "阿尔温", "费纳芬"};
    static const std::vector<std::string> orc_names = {"格罗姆", "乌格鲁克", "鲁兹", "夏格拉特", "阿佐格", "博尔多"};
    static const std::vector<std::string> halfling_names = {"比尔博", "弗罗多", "山姆", "皮平", "梅里", "罗索"};
    ...
    return (*pool)[dist(global_rng)];
}

std::string generate_commander_name(Race race) {
    static const std::vector<std::string> suffixes = {"将军", "统领", "勇士", "领主", "队长"};
    std::uniform_int_distribution<size_t> dist(0, suffixes.size() - 1);
    return generate_king_name(race) + "·" + suffixes[dist(global_rng)];
}
...
void generate_new_king(Kingdom& k, const std::array<double, 5>& old_personality, bool is_initial) {
    if (is_initial) {
        ...
        k.king_name = generate_king_name(k.race);
    } else {
...
int create_kingdom(std::vector<Kingdom>& kingdoms, const WorldData& world,
    ...
    oss << "新王国 \"" << new_kingdom.name << "\" 在 (" << capital_x << ", " << capital_y
        << ") 建立，种族：" << race_str;
    EventRecord event(turn, std::to_string(turn / 4) + "年",
...
void collect_resources(Kingdom& k, WorldData& world, int turn) {
    ...
    k.food += total_food - army_food_cost;
    k.gold += total_gold - building_maintenance - road_maintenance;
	...
	// 资源溢出损耗：当资源超过 80000 时，超出部分按 5% 比例衰减（避免长期饱和）
	constexpr double SURPLUS_THRESHOLD = 80000.0;
...
void grow_population(Kingdom& k, int turn) {
    ...
    double total_rate_factor = k.industriousness * k.growth_modifier() * k.cached_growth_mod;
    double growth_rate = base_rate * total_rate_factor;
    ...
    if (k.food >= 0.0) {
        ...
        if (total_pop <= 0) return;
        double famine_ratio = std::min(0.5, deficit / static_cast<double>(total_pop));
        for (auto& s : k.settlements) {
...
void manage_settlements(Kingdom& k, const WorldData& world, int turn) {
    ...
    if (buildings_built >= MAX_BUILDINGS_PER_TURN) return;
    ...
    for (const BuildCandidate& cand : candidates) {
        ...
        int pop_cost = 2;                            // 每建筑消耗2人口

        Settlement& s = k.settlements[si];

        // 检查资源是否足够
        if (k.gold < gold_cost) continue;
        if (k.food < food_cost) continue;
        // 必须保留至少 MIN_SETTLEMENT_POP (通常为1) 人口
        if (s.population < pop_cost + Balance::MIN_SETTLEMENT_POP) continue;
        ...
        // 建造建筑
        s.buildings.push_back(type);
        buildings_built++;
    }

    // 最后统一更新一次效果缓存（避免循环内多次更新）
    k.update_cached_effects();
}
// ============================================================
// 第 5 部分结束
...
// ---------- 领土扩张 ----------
void expand_territory(Kingdom& k, const WorldData& world, int turn) {
    // 最大可扩张格数 = min(army * 0.05, MAX_EXPANSION_PER_TURN)
    int max_expand = static_cast<int>(std::floor(k.army * 0.05));
    max_expand = std::min(max_expand, MAX_EXPANSION_PER_TURN);
    if (max_expand <= 0) return;

    // 检查资源是否足够支付至少一格
    if (k.gold < Balance::EXPAND_COST_GOLD || k.food < Balance::EXPAND_COST_FOOD) return;

    // 构建候选格子列表：与当前领土相邻、无主、陆地
    std::vector<Position> candidates;
    const int dx[4] = {1, -1, 0, 0};
    const int dy[4] = {0, 0, 1, -1};
    ...
    for (const Position& pos : candidates) {
        ...
        if (k.gold < Balance::EXPAND_COST_GOLD || k.food < Balance::EXPAND_COST_FOOD) break;

        k.gold -= Balance::EXPAND_COST_GOLD;
        k.food -= Balance::EXPAND_COST_FOOD;
...
void build_new_settlement(Kingdom& k, const WorldData& world, int turn) {
	...
	// --- 强制建城逻辑 ---
	// 如果领土超过 50 格，且定居点少于 3 个，则忽略阈值强制建城
	bool force_settle = (k.territory.size() > 50 && k.settlements.size() < 3);
	if (!force_settle && best.score <= Balance::NEW_SETTLEMENT_SCORE_THRESH) {
    	return;   // 不满足条件且未达到强制建城要求，放弃
	}	

	// 若强制建城，但最佳候选评分极低（< -10），为防止在极差位置建城，仍放弃
	if (force_settle && best.score < -10.0) {
    	return;
	}
    if (best.score <= Balance::NEW_SETTLEMENT_SCORE_THRESH) return;
    	
   	std::uniform_real_distribution<double> prob(0.0, 1.0);
    if (prob(global_rng) >= k.greed) return;

    k.gold -= Balance::NEW_SETTLEMENT_COST;

    Settlement new_sett(best.pos.x, best.pos.y, Balance::NEW_SETTLEMENT_POP,
    ...
    oss << k.name << " 在 (" << best.pos.x << ", " << best.pos.y << ") 建立了新居住区 "
        << new_sett.name << "，人口 " << new_sett.population;
    EventRecord event(turn, std::to_string(turn / 4) + "年",
...
void transfer_tech(Kingdom& from, Kingdom& to, int turn) {
    ...
    std::uniform_int_distribution<size_t> dist(0, transferable.size() - 1);
    TechID selected = transferable[dist(global_rng)];
    double spread_prob = Balance::TECH_SPREAD_PROB * (0.5 + from.diplomacy * 0.5) *
                         (1.0 + from.civilization_level * 0.1);
    std::uniform_real_distribution<double> prob(0.0, 1.0);
    if (prob(global_rng) < spread_prob) {
...
void check_revolution(Kingdom& k, int turn) {
    ...
    // 更换国名
    if (k.name.find("共和国") == std::string::npos) {
        k.name += "共和国";
    } else {
        k.name = "新" + k.name;
    }
...
void check_king_death(Kingdom& k, int turn) {
    ...
    if (k.king_age > Balance::KING_DEATH_AGE_BASE) {
        death_prob += (k.king_age - Balance::KING_DEATH_AGE_BASE) * Balance::KING_DEATH_AGE_PER_YEAR;
    }
    if (k.war_timer > 0) {
...
void update_civilization_level(Kingdom& k, int turn) {
    // 1. 计算文化健康度（文化向量的综合表现）
    double culture_avg = 0.0;
    for (int i = 0; i < CULTURE_DIM; ++i) culture_avg += k.culture[i];
    culture_avg /= CULTURE_DIM;
    
    // 文化标准差：衡量文化的“纯粹度”或“凝聚力”，方差越小文化越统一
    double culture_var = 0.0;
    for (int i = 0; i < CULTURE_DIM; ++i) {
        double d = k.culture[i] - culture_avg;
        culture_var += d * d;
    }
    culture_var /= CULTURE_DIM;
    double culture_cohesion = std::max(0.0, 1.0 - culture_var * 2.0); // 0~1
    
    // 文化健康度：平均值 × 凝聚力
    double target_culture_health = culture_avg * (0.5 + 0.5 * culture_cohesion);
    target_culture_health = std::max(0.1, std::min(2.0, target_culture_health));
    
    // 文化健康度平滑变化（不会突变，但会趋势性变化）
    k.culture_health += (target_culture_health - k.culture_health) * 0.02;
    ...
    // 文化贡献（权重最高）
    base_score += k.culture_health * 2.0;
    
    // 科技贡献
    base_score += k.tech_level * 0.3;
    
    // 稳定度贡献（稳定度越高，文明越稳固）
    base_score += (k.stability / 100.0) * 1.5;
    
    // 人口贡献（人口规模代表文明辐射力）
    int pop = k.total_population();
    base_score += std::min(1.0, pop / 500.0) * 0.5;
    
    // 领土贡献（文明的影响范围）
    base_score += std::min(1.0, k.territory.size() / 100.0) * 0.3;
    
    // 文明等级对应的阈值（逐级递增）
    double required_score = 3.0 + k.civilization_level * 1.5;
    // 1级: 4.5, 2级: 6.0, 3级: 7.5 ... 10级: 18.0
    
    // 3. 判定升级、维持或倒退
    if (base_score > required_score + 0.5 && k.civilization_level < MAX_CIVILIZATION_LEVEL) {
        // 升级
    ...
    else if (base_score < required_score - 1.0 && k.civilization_level > 0) {
        // 衰退：文明倒退
        k.civ_decline_timer++;
        if (k.civ_decline_timer > 10) { // 连续10回合低于阈值才倒退（避免频繁波动）
            k.civilization_level = std::max(0, k.civilization_level - 1);
            k.civ_decline_timer = 0;
            std::ostringstream oss;
    ...
    // 4. 文明等级的惩罚效果（直接修改王国状态，强化惩罚）
    double civ_penalty = 1.0 - k.civilization_level * 0.05; // 0~1之间，文明越低惩罚越大
    
    // 低文明：叛乱风险大幅上升
    k.cached_rebellion_risk_mod = 1.0 + (1.0 - civ_penalty) * 0.5;
    
    // 低文明：科技研究速度下降
    k.cached_research_mod *= (0.5 + 0.5 * civ_penalty);
    
    // 低文明：外交信任度下降
    k.diplomacy = std::max(0.0, k.diplomacy - (1.0 - civ_penalty) * 0.05);
    
    // 低文明：稳定度额外惩罚
    if (k.civilization_level < 3) {
        k.stability -= (3 - k.civilization_level) * 0.5;
...
void process_wars(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
    ...
    for (auto& wp : war_pairs) {
        ...
        for (int k = 0; k < max_attacks; ++k) {
            ...
            // ===== 新增：定义损失随机分布（用于后续人口损失） =====
            std::uniform_real_distribution<double> loss_def(8.0, 20.0);
            std::uniform_real_distribution<double> loss_atk(3.0, 10.0);
            ...
            if (effective_atk > effective_def) {
                ...
                if (new_control <= 0.0) {
                    ...
                    // ===== 原军队损失 =====
                    double loss_def_val = loss_def(global_rng);
                    double loss_atk_val = loss_atk(global_rng);
                    defender.army = std::max(0.0, defender.army - loss_def_val);
                    attacker.army = std::max(0.0, attacker.army - loss_atk_val);

                    // ===== 新增：人口损耗（征兵损耗） =====
                    int pop_loss_def = static_cast<int>(loss_def_val * 0.5);
                    int pop_loss_atk = static_cast<int>(loss_atk_val * 0.5);
                    // 防守方人口扣除
                    if (!defender.settlements.empty()) {
                        int total_pop = defender.total_population();
                        if (total_pop > 0) {
                            double ratio = static_cast<double>(pop_loss_def) / total_pop;
                            for (auto& s : defender.settlements) {
                                s.population = std::max(Balance::MIN_SETTLEMENT_POP,
                                                        s.population - static_cast<int>(s.population * ratio));
                            }
                        }
                    }
                    // 攻击方人口扣除
                    if (!attacker.settlements.empty()) {
                        ...
                        if (total_pop > 0) {
                            double ratio = static_cast<double>(pop_loss_atk) / total_pop;
                ...
                // 攻击失败：双方各损失少量兵力，模拟消耗战
                std::uniform_real_distribution<double> loss_atk_fail(1.0, 4.0);
                std::uniform_real_distribution<double> loss_def_fail(1.0, 3.0);
                attacker.army = std::max(0.0, attacker.army - loss_atk_fail(global_rng));
                defender.army = std::max(0.0, defender.army - loss_def_fail(global_rng));
                // ===== 新增：失败方也消耗金币 =====
...
// ---------- 辅助：时间格式化 ----------
std::string format_turn(int turn) {
    int year = turn / 4;
...
void write_kingdom_file(const Kingdom& k, const std::string& dir) {
    ...
    out << "建立回合: " << k.establish_turn << " (" << format_turn(k.establish_turn) << ")\n";
    if (!k.alive) out << "灭亡回合: " << k.death_turn << " (" << format_turn(k.death_turn) << ")\n";
    out << "----------------------------------------\n";
...
void write_fallen_kingdoms_file(const std::vector<Kingdom>& all_kingdoms, const std::string& dir) {
    ...
    for (const auto& k : all_kingdoms) {
        ...
        out << "   种族: " << race_name(k.race) << "\n";
        out << "   建立回合: " << k.establish_turn << " (" << format_turn(k.establish_turn) << ")\n";
        out << "   灭亡回合: " << k.death_turn << " (" << format_turn(k.death_turn) << ")\n";
        out << "   灭亡原因: ";
...
void recruit_army(Kingdom& k, int turn) {
    ...
    double max_recruitable = std::min(max_army - k.army, pop * 0.05);
    double gold_limit = k.gold / 2.0;
    double possible = std::min(max_recruitable, gold_limit);
    ...
    if (total_pop > 0 && !k.settlements.empty()) {
        double ratio = static_cast<double>(count) / total_pop;
        for (auto& s : k.settlements) {
...
void check_extinction(Kingdom& k, int turn) {
    ...
    std::ostringstream oss;
    oss << k.name << " 灭亡！";
    EventRecord event(turn, format_turn(turn), EventType::EXTINCTION, k.id, oss.str(), false);
    globalEventLog.push_back(event);
    std::cout << "[" << turn << "] " << oss.str() << std::endl;
    ...
    for (size_t i = 0; i < global_ruins.size(); ++i) {
        ...
        if (prob(global_rng) < 0.05) { // 每回合5%探索概率
            ...
            if (prob(global_rng) < success) {
                ...
                EventRecord event(turn, format_turn(turn), EventType::CULTURAL, k.id, oss.str(), false, r.x, r.y);
                globalEventLog.push_back(event);
                std::cout << "[" << turn << "] " << oss.str() << std::endl;
            }
...
void run_simulation(const std::string& world_file, const std::string& output_dir) {
    ...
    // ========== 时间系统变量 ==========
    int current_year = 0;
    int current_month = 0;          // 0=1月
    int total_months_passed = 0;
    const int MAX_MONTHS = MAX_TURNS * 3; // 15000月
    ...
    // 年度计数器
    int global_events_this_year = 0;

    std::uniform_int_distribution<int> day_dist(1, 30);
    std::uniform_real_distribution<double> prob(0.0, 1.0);
    ...
    while (total_months_passed < MAX_MONTHS) {
        // ---------- 决定下次事件间隔 ----------
        if (months_until_next_event <= 0) {
            if (prob(global_rng) < 0.8) {
                months_until_next_event = std::uniform_int_distribution<int>(1, 6)(global_rng);
            } else {
                months_until_next_event = std::uniform_int_distribution<int>(7, 12)(global_rng);
            }
            event_this_cycle = false;
        }

        // ---------- 推进时间 ----------
        int advance = std::min(months_until_next_event, MAX_MONTHS - total_months_passed);
        total_months_passed += advance;
        months_until_next_event -= advance;

        // 更新年月
        current_month += advance;
        while (current_month >= 12) {
            current_month -= 12;
            current_year++;
            global_events_this_year = 0; // 新年重置
        }

        // 计算当前回合数（每回合 = 3个月）
        int current_turn = total_months_passed / 3;
        ...
        // ---------- 显性事件 ----------
        bool year_has_events = true; // 默认有事件
        ...
        if (months_until_next_event == 0 && !event_this_cycle) {
            ...
            if (global_events_this_year <= 2 && prob(global_rng) < 0.15) {
                ...
                std::cout << "[" << current_year << "年] 这一年平安无事，风调雨顺。" << std::endl;
                // 注意：此处不 continue，王国行动仍会执行
            }

            // ---------- 只有 year_has_events == true 时才生成事件 ----------
            if (year_has_events) {
                // 生成随机日期
                int event_day = day_dist(global_rng);
                bool is_important = (prob(global_rng) < 0.15);
                std::string date_str = std::to_string(current_year + 1) + "年"
                                     + std::to_string(current_month + 1) + "月"
                                     + (is_important ? std::to_string(event_day) + "日" : "");
                ...
                for (auto& k : kingdoms) {
                    if (!k.alive) continue;
                    if (prob(global_rng) > 0.3) continue;
                    ...
                    if (k.stability < 30.0 && roll < 0.4) {
                        ...
                        handle_rebellion(k, current_turn);
                    }
                    // 军力强+好战 -> 扩张/战争（夏季更易）
                    ...
                    // 富裕 -> 经济繁荣（春季更易）
                        ...
                        std::cout << "[" << date_str << "] " << k.name << " 贸易繁荣，国库增加。" << std::endl;
                    }
                    // 其他事件（秋季丰收等）
                    else if (roll < 0.3) {
                        if (season == 3) {
                            k.food += 100;
                            std::cout << "[" << date_str << "] " << k.name << " 秋收大丰收，粮仓充实！" << std::endl;
                        } else {
                            for (int c = 0; c < CULTURE_DIM; ++c) {
                                k.culture[c] += (0.5 - k.culture[c]) * 0.05;
                            }
                            std::cout << "[" << date_str << "] " << k.name << " 文化繁荣，艺术兴盛。" << std::endl;
                        }
                    }
                    ...
                    if (is_important) {
                        std::ostringstream oss;
                        oss << "[" << date_str << "] " << k.name << " 发生重大事件！";
                        EventRecord event(current_turn, date_str,
                                          EventType::POLITICAL, k.id, oss.str(), false);
                        globalEventLog.push_back(event);
                    }
                }
                ...
                if (prob(global_rng) < 0.05) {
                    ...
                    EventRecord event(current_turn, date_str,
                                      EventType::INTERNATIONAL, -1, oss.str(), true);
                    globalEventLog.push_back(event);
                    std::cout << "[" << date_str << "] " << oss.str() << std::endl;
                }
        ...
        if (months_until_next_event == 0) {
            ...
            // 每个王国行动（使用 current_turn，即回合数）
            for (int kid : order) {
                Kingdom& k = kingdoms[kid];
                if (!k.alive) continue;

                k.update_cached_effects();
                collect_resources(k, world, current_turn);
                grow_population(k, current_turn);
                manage_settlements(k, world, current_turn);
                expand_territory(k, world, current_turn);
                build_new_settlement(k, world, current_turn);
                recruit_army(k, current_turn);
                update_technology(k, current_turn);
                update_stability(k, world, current_turn);
                handle_rebellion(k, current_turn);
                check_king_death(k, current_turn);
                check_revolution(k, current_turn);
                update_civilization_level(k, current_turn);
                build_roads(k, world, current_turn);
                maintain_roads(k, current_turn);
                explore_ruins(k, current_turn);
                process_domestic_events(k, current_turn);
                process_hidden_events(k, current_turn);
                apply_disasters(k, world, current_turn);
                apply_magic_disasters(k, current_turn);
                check_extinction(k, current_turn);
            }

            // 全局处理（使用 current_turn）
            if (current_turn % CULTURE_SPREAD_INTERVAL == 0) {
                spread_culture(kingdoms, world, current_turn);
            }
            recover_control(kingdoms, current_turn);
            update_alliances(kingdoms, world, current_turn);
            process_international_events(kingdoms, current_turn);
            propagate_tech_globally(kingdoms, current_turn);
            ally_join_wars(kingdoms, current_turn);
            process_wars(kingdoms, world, current_turn);
            update_diplomacy(kingdoms, world, current_turn);
            check_world_war(kingdoms, current_turn);
            check_for_new_kingdoms(current_turn, world, kingdoms);

            // 每20回合输出一次快照
            if (current_turn % OUTPUT_CHECKPOINT_INTERVAL == 0) {
                int alive = 0;
                for (const auto& k : kingdoms) if (k.alive) alive++;
                std::cout << "===== 第 " << current_year << " 年 " << current_month+1
                          << " 月 (回合 " << current_turn << ") | 存活王国: "
