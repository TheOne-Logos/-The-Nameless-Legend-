...
// ---------- 人口增长 ----------
void grow_population(Kingdom& k, int turn) {
    // ===== [修改] 基础增长率提升 + 文明等级0时不再过度惩罚 =====
    // 原逻辑：base_rate = 0.03
    // 新逻辑：base_rate = 0.05（每回合5%增长，类似复利）
    double base_rate = Balance::BASE_POP_GROWTH_RATE;
    
    // 确保 growth_mod 至少为 0.6，避免文明等级0时增长停滞
    ...
    // 即使所有系数都极低，也保证至少 0.5% 的基础增长（防止停滞）
    growth_rate = std::max(0.005, growth_rate);
    ...
    if (k.food >= 0.0) {
        ...
        // ===== [修改] 食物不足时，饥荒惩罚从 50% 降到 20% =====
        double deficit = -k.food;
        int total_pop = k.total_population();
        if (total_pop <= 0) return;
        double famine_ratio = std::min(0.2, deficit / static_cast<double>(total_pop)); // 0.5 → 0.2
        for (auto& s : k.settlements) {
...
void build_new_settlement(Kingdom& k, const WorldData& world, int turn) {
    ...
    if (k.greed <= 0.05) return;  // 贪婪度极低时不建城（除非强制）
    ...
    // 如果每个定居点平均管辖超过 30 格，或每个定居点平均人口超过 150 人，触发自动建城
    bool auto_settle = (territory_per_settlement > 30.0) || (pop_per_settlement > 150.0);

    // 如果触发自动建城，跳过所有评分和概率检查
    if (auto_settle) {
        // 查找最佳候选位置（即使评分低也要建）
        ...
        Settlement new_sett(best_pos.x, best_pos.y, Balance::NEW_SETTLEMENT_POP,
        ...
        // 记录事件，说明触发原因
        std::string reason;
        if (territory_per_settlement > 30.0 && pop_per_settlement > 150.0) {
            reason = "领土过大且人口过多";
        } else if (territory_per_settlement > 30.0) {
        ...
        oss << k.name << " 因" << reason << "在 (" << best_pos.x << ", " << best_pos.y 
            << ") 自动建立了新居住区 " << new_sett.name << "，人口 " << new_sett.population;
        EventRecord event(turn, format_time(turn),
    ...
    // 宽松的强制建城条件（领土压力或人口压力已在上面的自动建城处理，这里用于非自动情况）
    // 但为了进一步保证，仍然保留强制条件
    bool force_settle = false;
    if (k.territory.size() > 30 && k.settlements.size() < 3) force_settle = true;
    if (k.territory.size() > 50 && k.settlements.size() < 5) force_settle = true;
    if (k.total_population() > 200 && k.settlements.size() < 3) force_settle = true;
...
void update_civilization_level(Kingdom& k, int turn) {
    ...
    // ===== [修改] 人口贡献：提高权重，降低门槛 =====
    // 原：std::min(1.0, pop / 500.0) * 0.5
    // 新：std::min(1.0, pop / 200.0) * 0.8
    int pop = k.total_population();
    base_score += std::min(1.0, pop / 200.0) * 0.8;
    ...
    // ===== [修改] 文明等级升降阈值调整（进一步放宽升级条件） =====
    // 原升级条件：base_score > required_score + 2.0  →  过于严格
    // 现升级条件：base_score > required_score + 0.5  →  稍微超出即可升级
    // 降级条件保持：base_score < required_score - 0.5 且连续5回合
    ...
    // ===== [修改] 根据文明等级计算 civ_effect_mod（提升最小系数） =====
    // 原：min_mod=0.5, max_mod=1.5
    // 新：min_mod=0.7, max_mod=1.5
    double min_mod = 0.7;
    double max_mod = 1.5;
