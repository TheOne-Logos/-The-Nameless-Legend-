...
void collect_resources(Kingdom& k, WorldData& world, int turn) {
    ...
    // ===== [修改] 税收与腐败：引入行政损耗和稳定度影响 =====
    // 原公式：double tax_income = k.total_population() * k.tax_rate * k.cached_tax_mod;
    // 新公式：考虑腐败（领土越大效率越低）和稳定度（低稳定度税收减少）
    double corruption_factor = 1.0 / (1.0 + static_cast<double>(k.territory.size()) / 500.0);
    double stability_tax_factor = 0.5 + (k.stability / 200.0);  // 稳定度0→0.5, 100→1.0
    ...
    // ---------- 资源溢出损耗（调整阈值，避免过度削平） ----------
    constexpr double SURPLUS_THRESHOLD = 90000.0;   // 原来是80000，提高到90000
    constexpr double DECAY_RATE = 0.05;             // 5%衰减
...
void manage_settlements(Kingdom& k, const WorldData& world, int turn) {
    ...
    for (int i = 0; i < max_to_consider; ++i) {
        ...
        pop_cost_per_settlement[idx] += 2;  // 每建筑消耗2人口
    }
    ...
    for (const BuildCandidate& cand : candidates) {
        ...
        k.gold -= gold_cost;
        k.food -= food_cost;
        s.population -= 2;      // 扣除人口（已通过批量检查确保安全）
        s.buildings.push_back(cand.type);
        buildings_built++;
        // 减少该定居点剩余计划建造计数
        pop_cost_per_settlement[si] -= 2;
    }
...
void expand_territory(Kingdom& k, const WorldData& world, int turn) {
    ...
    for (const Position& pos : candidates) {
        ...
        // ===== [修改] 动态扩张成本：领土越大，扩张越贵 =====
        double size_factor = 1.0 + (static_cast<double>(k.territory.size()) / 100.0);
        double gold_cost = Balance::EXPAND_COST_GOLD * size_factor;
        double food_cost = Balance::EXPAND_COST_FOOD * size_factor;

        // 检查资源是否足够支付当前格的成本
        if (k.gold < gold_cost || k.food < food_cost) break;
...
void update_civilization_level(Kingdom& k, int turn) {
    ...
    // ===== [修改] 文明等级升降阈值调整 =====
    // 原升级条件：base_score > required_score + 0.5  →  过于宽松
    // 现升级条件：base_score > required_score + 2.0  →  需要明显优势才能升级
    // 原降级条件：base_score < required_score - 1.0 且连续10回合 →  过于严苛
    // 现降级条件：base_score < required_score - 0.5 且连续5回合  →  更易衰退
    if (base_score > required_score + 2.0 && k.civilization_level < MAX_CIVILIZATION_LEVEL) {
        // 升级
    ...
    else if (base_score < required_score - 0.5 && k.civilization_level > 0) {
        // 衰退：文明倒退（观察期为 5 回合，原来是 10 回合）
        k.civ_decline_timer++;
        if (k.civ_decline_timer > 5) {
    ...
    // 4. 根据文明等级计算 civ_effect_mod（持久化系数）
    // 等级0对应0.5，等级10对应1.5，线性插值，并限制范围
    double min_mod = 0.5;
    double max_mod = 1.5;
...
void process_wars(std::vector<Kingdom>& kingdoms, const WorldData& world, int turn) {
    ...
    for (auto& wp : war_pairs) {
        ...
        for (int k = 0; k < max_attacks; ++k) {
            ...
            // ===== [修改] 战争伤亡提升至“伤筋动骨”级别 =====
            // 原来：loss_def(8.0, 20.0), loss_atk(3.0, 10.0)
            // 现在：绝对值放大 10 倍，并额外增加 2% 的百分比损耗
            ...
            if (effective_atk > effective_def) {
                ...
                if (new_control <= 0.0) {
                    ...
                    // ===== [FIX] 检查易主的格子是否存在防守方的定居点，若有则强制移除（防止幽灵王国） =====
                    ...
                    // ===== [修改] 兵力损失：绝对值 + 百分比（当前兵力的 2%） =====
                    double loss_def_val = loss_def(global_rng);
                    double loss_atk_val = loss_atk(global_rng);
                    double percent_loss_def = defender.army * 0.02;  // 额外损失 2%
                    double percent_loss_atk = attacker.army * 0.02;
                ...
                double percent_loss_def = defender.army * 0.01;  // 失败时损耗减半（1%）
