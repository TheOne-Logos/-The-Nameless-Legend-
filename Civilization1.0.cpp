...
// ---------- 文明等级更新（文明点数积累制） ----------
void update_civilization_level(Kingdom& k, int turn) {
    if (k.civilization_level >= MAX_CIVILIZATION_LEVEL) return;

    // 计算每回合获得的文明点数
    double base_gain = 0.02; // 基础增长，略微提高让前期不会太慢

    // 学院加成
    double academy_bonus = 0.0;
    for (const auto& s : k.settlements) {
        for (BuildingType bt : s.buildings) {
            if (bt == BuildingType::ACADEMY) academy_bonus += 0.025;
            else if (bt == BuildingType::UNIVERSITY) academy_bonus += 0.05;
            else if (bt == BuildingType::ROYAL_ACADEMY) academy_bonus += 0.08;
            else if (bt == BuildingType::SAGE_HALL) academy_bonus += 0.03;
            else if (bt == BuildingType::LIBRARY) academy_bonus += 0.015;
        }
    }

    // 科技等级加成（科技越高，文明积累越快）
    double tech_bonus = k.tech_level * 0.015;

    // 稳定度加成（稳定度高于50才有正向加成，低于50会扣分）
    double stab_bonus = (k.stability - 50.0) * 0.002;

    // 人口规模加成（人口越多，文明越繁荣）
    double pop_bonus = std::min(0.05, k.total_population() * 0.00001);

    // 文化维度加成（四个维度平均值越高越好）
    double culture_avg = 0.0;
    for (int i = 0; i < CULTURE_DIM; ++i) culture_avg += k.culture[i];
    culture_avg /= CULTURE_DIM;
    double culture_bonus = (culture_avg - 0.3) * 0.02;
    ...
    double war_penalty = at_war ? -0.03 : 0.0;

    // 计算总获得
    double gain = base_gain + academy_bonus + tech_bonus + 
                  std::max(0.0, stab_bonus) + pop_bonus + 
                  std::max(0.0, culture_bonus) + war_penalty;
    ...
    // 计算升级所需点数（逐级递增）
    double required = 8.0 + k.civilization_level * 4.0;
    // 1级要 8分，2级要 12分，3级要 16分 ... 10级要 44分

    // 点数达标则升级
    if (k.civ_points >= required) {
        k.civilization_level++;
        k.civ_points = 0.0; // 升级后清零
...
void run_simulation(const std::string& world_file, const std::string& output_dir) {
    ...
    generate_ruins(world, 20); // 生成20个遗迹
    ...
    for (int turn = 0; turn < MAX_TURNS; ++turn) {
        ...
        // 全局处理
        if (turn % CULTURE_SPREAD_INTERVAL == 0) spread_culture(kingdoms, world, turn);
        ...
        if (turn % OUTPUT_CHECKPOINT_INTERVAL == 0) {
            ...
            std::cout << "===== 回合 " << turn << " | 存活王国: " << alive << "/" << kingdoms.size() << " =====" << std::endl;
        }
    }
