...
void update_civilization_level(Kingdom& k, int turn) {
    ...
    if (base_score > required_score + 0.5 && k.civilization_level < MAX_CIVILIZATION_LEVEL) {
        ...
        // 注意：此处使用 format_time(turn) 为统一时间格式，但该函数尚未定义，可先临时用 to_string，后续第7步统一替换
        EventRecord event(turn, format_time(turn), // 临时，待统一
    ...
    // 注意：不再直接修改 cached_* 或 diplomacy/stability
    // 所有影响将通过 civ_effect_mod 在 update_cached_effects() 中统一体现
}
// ============================================================
// 第 8 部分结束
// ============================================================
